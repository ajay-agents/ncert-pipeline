# -*- coding: utf-8 -*-
"""Stage 9 render script for physics-12-13 (Nuclei / नाभिक): reads
...
chapters/physics-12-12/09_design/render_page.py (the most recently proven
and complete of this pipeline's stage-9 scripts), and writes
09_design/final.hi.html + final.hi.css.

This chapter has no :::table or :::figure content anywhere in its final item
set (14 items: 4 examples + 10 exercises; confirmed at stage 2 that this
chapter's 2 mathpix images are never referenced inside any example/exercise -
0 figures used) but DOES have exactly one :::note (ex_13.1's neutron-star
remark, type="tip") - so the note branch below is genuinely exercised here,
unlike physics-12-12's own run. The table/figure branches are inherited
unused, kept only so this script stays a faithful copy of the proven
reference rather than a stripped-down one that might silently diverge in
behavior it does share.
"""
...
def render_block_flow(flow):
    ...
    exercised by this chapter's own content (0 figures, 0 tables) but kept
    for parity with the reference script."""
    parts = []
    for seg in flow:
        if seg["type"] == "text":
...
def render_item(item, color_idx):
    ...
    if item["parts"]:
        ...
        # non-empty parts (the known duplicate-solution artifact). Neither of
        # this chapter's two multi-part items (ex_13.4, q_13.5) has one - both
        # confirmed empty at stage 7/8 - so this branch is a no-op here, kept
        # for parity with the reference script.
    ...
    # item-level combined answer alongside parts - this chapter's two
    # multi-part items both have empty item-level "answer" (confirmed at
    # stage 7/8), so this never fires; kept for parity with the reference
    # script / a future rerun.
