# -*- coding: utf-8 -*-
"""Stage 9 render script for maths-12-3-en (Matrices, English medium),
adapted from two lineages: physics-12-5-en's render_page.py (the most
recently built in this repo, English wording, section titles, the more
evolved `strip_redundant_enumeration` with telescoped-enumeration handling
and a decimal-point guard, the `any_part_has_solution` fallback for a
multi-part item whose real solution lives only at item level, and the
`.s98`-wrapped table CSS convention) for its general/English-language
machinery; maths-12-3's/maths-12-4's own render_page.py (the Hindi-medium
converters for THIS SAME chapter and its sibling, Determinants) for the
note-block group-kind handling (this chapter has exactly one :::note,
ex_3.12's "AB defined does not mean BA defined" remark) already fixed there.

This chapter has ZERO figures anywhere (confirmed at stage 8 - a Maths
chapter, not Physics/Biology) - the figure-rendering branches inherited from
physics-12-5-en's render_page.py are kept for parity/robustness (a future
...
Known edge cases from this chapter's own 08_tag manifest notes, rendered
faithfully here rather than re-detected:
- q_3.42 (and a handful of similar multi-part items, e.g. q_3.11) has ONE
  shared item-level solution_blocks list covering all of its parts inline,
  with every part's own solution_blocks left empty - the
  `any_part_has_solution` fallback below (inherited from physics-12-5-en)
  is exactly what renders this correctly: every part's own prompt is shown
  under its own "Part (i)" heading, then the single combined item-level
  solution stack is rendered once, after all the part prompts, instead of
  being silently dropped for living "one level up" from where a naive
  per-part loop would look.
- sol_3.misc.12's two induction proofs (each originally an outer grouping
  step around three inner named steps) were already flattened by stage 8
  into a single ordered solution_blocks list - rendered here exactly in the
  order and with the stage values the JSON already provides, no special
  casing needed.
"""
import json
import re
...
# 3 times (caught for real - ex_3.19's own conclusion split "i.e." into two
# stray one-word "sentences", "i." then "e.", exactly the class of bug
# step_9/PROMPT.md documents for an abbreviation the sentence-splitter
# doesn't know about); "e.g."/"etc."/"Fig." occur zero times here but cost
# nothing to guard against defensively (matching physics-12-5-en's own
# abbreviation set) in case a later correction pass introduces one.
...
def render_table_html(table_md):
    ...
    this chapter's only two tables (ex_3.1's factory men/women-workers data,
    q_3.52's sales data) both live inside their item's own :::prompt rather
    than inside a solution step (per the stage-8 manifest note); a
    flow-reader that only ever joins type:"text" segments would silently
    drop these, the same documented blind spot as a dropped mid-solution
    figure. Wrapped in a scrollable .s98 div so a wide table never overflows
...
def render_item(item, color_idx, badges):
    ...
    if parts_list:
        # Some items (q_3.42, q_3.11 and similar - see this chapter's own
        # 08_tag manifest notes) have parts whose OWN solution_blocks/answer
        # are empty, with the real combined solution addressing every part
        # together living only at item level (positional stage-inference
        # only fires once, item-wide, since there's no way to detect "(i)/
        # (ii)/(iii)/(iv)" sub-boundaries from the flat container tree).
...
def build_page():
    ...
    sections = [
        ...
        # This chapter keeps 21 genuine orphan solutions (a prior-edition
        # exercise no longer in the current textbook, kept per the
        # "nothing invented" rule) as their own additional_exercise items -
        # a light "Additional Questions" divider matching the same visual
        # weight as the other two section dividers, the same convention
        # used for physics-12-5-en's own 15 orphan items.
