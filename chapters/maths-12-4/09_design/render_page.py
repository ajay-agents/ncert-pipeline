# -*- coding: utf-8 -*-
"""Stage 9 render script for maths-12-4 (सारणिक / Determinants), adapted
from maths-12-3's own working render_page.py/latex.py (the most recent,
most similar prior chapter), plus every trap documented in step_9/PROMPT.md.
Section list rewritten for this chapter's five exercises (प्रश्नावली
4.1-4.5) instead of maths-12-3's three; note-block rendering added (this
chapter has one genuine :::note, ex_4.10's टिप्पणी remark - maths-12-3 had
none, so that path was never exercised there); no tables in this chapter
(0 :::table occurrences, unlike maths-12-3's q_3.9.7); determinant-bar
rendering lives in latex.py alongside the inherited bracketed-matrix
rendering (see latex.py's own module docstring)."""
import json
import re
import sys
import os
...
def build_page():
    ...
    sections = [
        ...
        ('प्रश्नावली 4.1', [it for it in ITEMS if it['id'].startswith('q_4.1.')]),
        ('प्रश्नावली 4.2', [it for it in ITEMS if it['id'].startswith('q_4.2.')]),
        ('प्रश्नावली 4.3', [it for it in ITEMS if it['id'].startswith('q_4.3.')]),
        ('प्रश्नावली 4.4', [it for it in ITEMS if it['id'].startswith('q_4.4.')]),
        ('प्रश्नावली 4.5', [it for it in ITEMS if it['id'].startswith('q_4.5.')]),
...
<title>सारणिक · अध्याय 4 &mdash; प्रश्न एवं हल</title>
