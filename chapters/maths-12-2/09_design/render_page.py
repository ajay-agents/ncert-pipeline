# -*- coding: utf-8 -*-
"""Stage 9 render script for maths-12-2 (Inverse Trigonometric Functions).
Modeled on maths-12-1's own stage-9 output shape (its render_page.py/
latex.py were not preserved in that chapter's own directory, so this was
rebuilt from its final.hi.html/.css as the reference, plus every trap
...
def render_prompt(text):
    ...
    return convert_math_spans(joined)
...
def render_solution_stack(blocks, answer_text, color_idx):
    ...
    for i, g in enumerate(groups):
        ...
        body_html = ''.join(
            render_prose_lines(flow_text(b['flow'])) for b in g['blocks']
        )
...
def render_item(item, color_idx, badges):
    ...
    body = []
    if prompt_text_use:
        body.append(f'<p class="s21">{render_prompt(prompt_text_use)}</p>')
    ...
    if parts_list:
        for p in parts_list:
            ...
            if p['prompt_text']:
                body.append(f'<p class="s21">{render_prompt(p["prompt_text"])}</p>')
...
def build_page():
    ...
    sections = [
        ...
        ('प्रश्नावली 2.1', [it for it in ITEMS if it['id'].startswith('q_2.1.')]),
        ('प्रश्नावली 2.2', [it for it in ITEMS if it['id'].startswith('q_2.2.')]),
        ('विविध प्रश्नावली', [it for it in ITEMS if it['kind'] == 'additional_exercise']),
    ]

    out.append('<div class="s2">\n  <div class="s3">प्रतिलोम त्रिकोणमितीय फलन</div>\n  <div class="s4">अध्याय 2 · सभी प्रश्न एवं पूर्ण हल</div>\n  <div class="s5"><span class="s6"></span><span class="s7"></span><span class="s8"></span><span class="s9"></span></div>\n</div>\n')
...
<title>प्रतिलोम त्रिकोणमितीय फलन · अध्याय 2 &mdash; प्रश्न एवं हल</title>
