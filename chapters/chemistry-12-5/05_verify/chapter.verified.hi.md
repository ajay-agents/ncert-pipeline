---
subject: chemistry
class: 12
chapter: 5
lang: hi
title: "उपसहसंयोजन यौगिक"
---

# उपसहसंयोजन यौगिक

## उदाहरण

:::example{number="5.1" kind="example" id="ex_5.1" topic="Secondary valency from AgCl precipitation data"}
#### उदाहरण 5.1

:::prompt
जलीय विलयनों में किए गए निम्नलिखित प्रेक्षणों के आधार पर निम्नलिखित यौगिकों में धातुओं की द्वितीयक संयोजकता बतलाइए।

सूत्र
आधिक्य में $\mathrm{AgNO}_{3}$ मिलाने पर एक मोल यौगिक से अवक्षेपित $\mathbf{A g C l}$ के मोलों की संख्या

(i) $\mathrm{PdCl}_{2} \cdot 4 \mathrm{NH}_{3}$ — $2$
(ii) $\mathrm{NiCl}_{2} \cdot 6 \mathrm{H}_{2} \mathrm{O}$ — $2$
(iii) $\mathrm{PtCl}_{4} \cdot 2 \mathrm{HCl}$ — $0$
(iv) $\mathrm{CoCl}_{3} \cdot 4 \mathrm{NH}_{3}$ — $1$
(v) $\mathrm{PtCl}_{2} \cdot 2 \mathrm{NH}_{3}$ — $0$
:::

:::solution{label="हल"}
(i) द्वितीयक संयोजकता $4$
(ii) द्वितीयक संयोजकता $6$
(iii) द्वितीयक संयोजकता $6$
(iv) द्वितीयक संयोजकता $6$
(v) द्वितीयक संयोजकता $4$
:::

:::

:::example{number="5.2" kind="example" id="ex_5.2" topic="Formulas of coordination compounds from names" corrections_applied="1"}
#### उदाहरण 5.2

:::prompt
निम्नलिखित उपसहसंयोजन यौगिकों के सूत्र लिखिए-

(i) टेट्राऐम्मीनएक्वाक्लोरिडोकोबाल्ट(III)क्लोराइड
(ii) पोटैशियम टेट्राहाइड्रॉक्सिडोजिंकेट(II)
(iii) पोटैशियम ट्राइऑक्सैलेटोऐलुमिनेट(III)
(iv) डाइक्लोरिडोबिस(एथेन-1, 2-डाइऐमीन)कोबाल्ट(III)
(v) टेट्राकार्बोनिलनिकल(0)
:::

:::solution{label="हल"}
(i) $\left[\mathrm{Co}\left(\mathrm{NH}_{3}\right)_{4}\left(\mathrm{H}_{2} \mathrm{O}\right) \mathrm{Cl}\right] \mathrm{Cl}_{2}$
(ii) $\mathrm{K}_{2}\left[\mathrm{Zn}(\mathrm{OH})_{4}\right]$
(iii) $\mathrm{K}_{3}\left[\mathrm{Al}\left(\mathrm{C}_{2} \mathrm{O}_{4}\right)_{3}\right]$
(iv) $\left[\mathrm{CoCl}_{2}(\mathrm{en})_{2}\right]^{+}$
(v) $\left[\mathrm{Ni}(\mathrm{CO})_{4}\right]$
:::

:::

:::example{number="5.3" kind="example" id="ex_5.3" topic="IUPAC names of coordination compounds"}
#### उदाहरण 5.3

:::prompt
निम्नलिखित उपसहसंयोजन यौगिकों के IUPAC नाम लिखिए-

(i) $\left[\mathrm{Pt}\left(\mathrm{NH}_{3}\right)_{2} \mathrm{Cl}\left(\mathrm{NO}_{2}\right)\right]$
(ii) $\mathrm{K}_{3}\left[\mathrm{Cr}\left(\mathrm{C}_{2} \mathrm{O}_{4}\right)_{3}\right]$
(iii) $\left[\mathrm{CoCl}_{2}(\text { en })_{2}\right] \mathrm{Cl}$
(iv) $\left[\mathrm{Co}\left(\mathrm{NH}_{3}\right)_{5}\left(\mathrm{CO}_{3}\right)\right] \mathrm{Cl}$
(v) $\mathrm{Hg}\left[\mathrm{Co}(\mathrm{SCN})_{4}\right]$
:::

:::solution{label="हल"}
(i) डाइऐम्मीनक्लोरिडोनाइट्रिटो-N-प्लैटिनम (II)
(ii) पोटैशियम ट्राइऑक्सैलेटोक्रोमेट (III)
(iii) डाइक्लोरिडोबिस(एथेन-1, 2-डाइऐमीन)कोबाल्ट (III)क्लोराइड
(iv) पेन्टाऐम्मीनकार्बोनेटोकोबाल्ट (III) क्लोराइड
(v) मर्क्यूरी (I) टेट्राथायोसायनेटो-S-कोबाल्टेट (III)
:::

:::

:::example{number="5.4" kind="example" id="ex_5.4" topic="No geometrical isomerism in tetrahedral complexes"}
#### उदाहरण 5.4

:::prompt
वे चतुष्फलकीय संकुल जिनमें दो भिन्न प्रकार के एकदंतुर लिगन्ड केंद्रीय धातु आयन से जुड़े हों, ज्यामितीय समावयवता क्यों नहीं दर्शाते?
:::

:::solution{label="हल"}
चतुष्फलकीय संकुल ज्यामितीय समावयवता नहीं दर्शाते, क्योंकि इनमें केंद्रीय धातु परमाणु से जुड़े एकदंतुर लिगन्डों की सापेक्ष स्थितियाँ आपस में एक जैसी होती हैं।
:::

:::

:::example{number="5.5" kind="example" id="ex_5.5" topic="Geometric isomers of [Fe(NH3)2(CN)4]-"}
#### उदाहरण 5.5

:::prompt
$\left[\mathrm{Fe}\left(\mathrm{NH}_{3}\right)_{2}(\mathrm{CN})_{4}\right]^{-}$के ज्यामितीय समावयवों की संरचनाएं दर्शाइए।
:::

:::figure{src="images/fig_5_84.svg" id="fig_5_84"}
:::

:::figure{src="images/fig_5_85.svg" id="fig_5_85"}
:::

:::solution{label="हल"}
समपक्ष समावयव




विपक्ष समावयव
:::

:::

:::example{number="5.6" kind="example" id="ex_5.6" topic="Chirality of cis and trans [CrCl2(ox)2]3-"}
#### उदाहरण 5.6

:::prompt
निम्नलिखित दो उपसहसंयोजन सत्ता में से कौन-सा काइरल (ध्रुवण घूर्णक) है?

(क) समपक्ष $-\left[\mathrm{CrCl}_{2}(\mathrm{ox})_{2}\right]^{3-}$
(ख) विपक्ष $-\left[\mathrm{CrCl}_{2}(\mathrm{ox})_{2}\right]^{3-}$
:::

:::figure{src="images/fig_5_86.svg" id="fig_5_86"}
:::

:::figure{src="images/fig_5_87.svg" id="fig_5_87"}
:::

:::solution{label="हल"}
ये दो उपसहसंयोजन सत्ता निम्न प्रकार से प्रदर्शित की जा सकती हैं -




(क) समपक्ष $\left[\mathrm{CrCl}_{2}(\mathrm{ox})_{2}\right]^{3-}$




(ख) विपक्ष $\left[\mathrm{CrCl}_{2}(\mathrm{ox})_{2}\right]^{3-}$
:::

:::answer
**उत्तर:** इन दोनों में से (क) समपक्ष- $\left[\mathrm{CrCl}_{2}(\mathrm{ox})_{2}\right]^{3-}$ काइरल (ध्रुवण घूर्णक) है।
:::

:::

:::example{number="5.7" kind="example" id="ex_5.7" topic="Geometry of [MnBr4]2- from magnetic moment"}
#### उदाहरण 5.7

:::prompt
$\left[\mathrm{MnBr}_{4}\right]^{2-}$ के 'केवल-प्रचक्रण' चुंबकीय आघूर्ण का मान 5.9 BM है। संकुल आयन की ज्यामिति बतलाइए।
:::

:::solution{label="हल"}
चूँकि संकुल आयन में $\mathrm{Mn}^{2+}$ आयन की समन्वय संख्या $4$ है, अतः यह या तो चतुष्फलकीय ( $s p^{3}$ संकरण ) या वर्गसमतल ( $d s p^{2}$ संकरण ) होगा। परंतु इस संकुल आयन का चुंबकीय आघूर्ण 5.9 BM है अतः $d$ कक्षकों में पाँच अयुगलित इलेक्ट्रॉनों की उपस्थिति के कारण इसकी आकृति चतुष्फलकीय होनी चाहिए न कि वर्ग समतलीय।
:::

:::

## प्रश्न और हल

:::question{number="5.1" kind="exercise" id="q_5.1" topic="Werner's postulates for bonding" corrections_applied="3"}
#### प्रश्न 5.1

:::prompt
वर्नर की अभिधारणाओं के आधार पर उपसहसंयोजन यौगिकों में आबंधन को समझाइए।
:::

:::figure{src="images/fig_5_88.svg" id="fig_5_88"}
:::

:::solution{label="हल"}
वर्नर के सिद्धान्त की मुख्य अभिधारणाएँ निम्न है।

1. उपसहसंयोजन यौगिकों में धातुएँ दो प्रकार की संयोजकताएँ दर्शाती है प्राथमिक तथा द्वितीयक।
2. प्राथमिक संयोजकताएँ सामान्य रूप से आयननीय होती हैं तथा ऋणात्मक आयनों द्वारा संतुष्ट होती हैं।

3. द्वितीयक संयोजकताएँ अन-आयननीय होती हैं। ये उदासीन अणुओं अथवा ऋणात्मक आयनों द्वारा संतुष्ट होती हैं। द्वितीयक संयोजकता उपसहसंयोजन संख्या के बराबर होती है तथा इसका मान किसी धातु के लिए सामान्यतः निश्चित होता है।
4. धातु से द्वितीयक संयोजकता से आबंधित आयन/समूह विभिन्न उपसहसंयोजन संख्या के अनुरूप दिक्स्थान से विशिष्ट रूप से व्यवस्थित रहते हैं। उदाहरण $\left[\mathrm{CoCl}_{2}\left(\mathrm{NH}_{3}\right)_{4}\right]^{+} \mathrm{Cl}^{-}$की संरचना निम्न प्रकार है।
:::

:::

:::question{number="5.2" kind="exercise" id="q_5.2" topic="Testing metal ions in coordination vs simple salts"}
#### प्रश्न 5.2

:::prompt
$\mathrm{FeSO}_{4}$ विलयन तथा $\left(\mathrm{NH}_{4}\right)_{2} \mathrm{SO}_{4}$ विलयन का $1: 1$ मोलर अनुपात में मिश्रण $\mathrm{Fe}^{2+}$ आयन का परीक्षण देता है परंतु $\mathrm{CuSO}_{4}$ व जलीय अमोनिया का $1: 4$ मोलर अनुपात में मिश्रण $\mathrm{Cu}^{2+}$ आयनो का परीक्षण नहीं देता। समझाइए क्यों?
:::

:::solution{label="हल"}
$\mathrm{FeSO}_{4}(\mathrm{aq})+\left(\mathrm{NH}_{4}\right)_{2} \mathrm{SO}_{4}(\mathrm{aq}) \longrightarrow \mathrm{FeSO}_{4} \cdot \underset{\text { मोहर लवण }}{\left(\mathrm{NH}_{4}\right)_{2} \mathrm{SO}_{4} \cdot 6 \mathrm{H}_{2} \mathrm{O}}$

$$
\begin{aligned}
\mathrm{FeSO}_{4} \cdot\left(\mathrm{NH}_{4}\right)_{2} \mathrm{SO}_{4} \cdot 6 \mathrm{H}_{2} \mathrm{O} \stackrel{(a q)}{\rightleftharpoons} \mathrm{Fe}^{2+}(\mathrm{aq})+2 \mathrm{NH}_{4}^{+}(\mathrm{aq}) \\
+2 \mathrm{SO}_{4}^{2-}(\mathrm{aq})+6 \mathrm{H}_{2} \mathrm{O}
\end{aligned}
$$

क्योंकि मोहर लवण के जल में घुलने से $\mathrm{Fe}^{2+}$ आयन प्राप्त होते हैं अतः इसका जलीय विलयन $\mathrm{Fe}^{2+}$ आयनों का परीक्षण देता है।
लेकिन जब $\mathrm{CuSO}_{4}(\mathrm{aq})$ को $\mathrm{NH}_{3}(\mathrm{aq})$ में मिलाते हैं जो संकुल बनता है।

$$
\mathrm{CuSO}_{4}(\mathrm{aq})+\mathrm{NH}_{3}(\mathrm{aq}) \longrightarrow\left[\mathrm{Cu}\left(\mathrm{NH}_{3}\right)_{4}\right] \mathrm{SO}_{4}
$$

यह संकुल जल में घुलकर $\mathrm{Cu}^{2+}$ आयन नहीं देता है क्योंकि यह संकुल आयन, $\left[\mathrm{Cu}\left(\mathrm{NH}_{3}\right)_{4}\right]^{2+}$ का अंश (भाग) हैं अतः $\mathrm{CuSO}_{4}$ और $\mathrm{NH}_{3}$ का विलयन $\mathrm{Cu}^{2+}$ आयनों का परीक्षण नहीं देता है।
:::

:::

:::question{number="5.3" kind="exercise" id="q_5.3" topic="Basic coordination terminology with examples" corrections_applied="2"}
#### प्रश्न 5.3

:::prompt
प्रत्येक के दो उदाहरण देते हुए निम्नलिखित को समझाइए- समन्वय समूह, लिगन्ड, उपसहसंयोजन संख्या, उपसहसंयोजन बहुफलक, होमोलेप्टिक तथा हेट्रोरोलेप्टिक।
:::

:::figure{src="images/fig_5_89.svg" id="fig_5_89"}
अष्टफलकीय
:::

:::figure{src="images/fig_5_90.svg" id="fig_5_90"}
चतुष्फलकीय
:::

:::figure{src="images/fig_5_91.svg" id="fig_5_91"}
वर्ग समतली
:::

:::figure{src="images/fig_5_92.svg" id="fig_5_92"}
त्रिकोणीय पिरैमिडी
:::

:::figure{src="images/fig_5_93.svg" id="fig_5_93"}
वर्ग पिरैमिडी
:::

:::figure{src="images/fig_5_94.svg" id="fig_5_94"}
:::

:::solution{label="हल"}
(i) समन्वय सत्ता केन्द्रीय धातु परमाणु अथवा आयन से किसी एक निश्चित संख्या में आबंधित आयन अथवा अणु मिलकर एक उपसहसंयोजन सत्ता अथवा समन्वय सत्ता का निर्माण करते हैं। उदाहरण $\left[\mathrm{CoCl}_{3}\left(\mathrm{NH}_{3}\right)_{3}\right]$, $\left[\mathrm{Ni}(\mathrm{CO})_{4}\right]$ आदि।

(ii) लिगेण्ड उपसहसंयोजन सत्ता में केन्द्रीय परमाणु/आयन से परिबद्ध आयन अथवा अणु लिगेण्ड कहलाते हैं। ये (i) सामान्य आयन हो सकते हैं जैसे $\mathrm{Cl}^{-}$; (ii) छोटे अणु हो सकते हैं, जैसे $\mathrm{H}_{2} \mathrm{O}$ या $\mathrm{NH}_{3}$, (iii) बड़े अणु हो सकते हैं जैसे $\mathrm{H}_{2} \mathrm{NCH}_{2} \mathrm{CH}_{2} \mathrm{NH}_{2}$ या $\mathrm{N}\left(\mathrm{CH}_{2} \mathrm{CH}_{2} \mathrm{NH}_{2}\right)_{3}$ अथवा (iv) बृहदणु भी हो सकते हैं जैसे प्रोटीन। उपसहसंयोजन के लिए उपलब्ध दाता परमाणुओं की संख्या के आधार पर लिगेण्डों को निम्न प्रकार वर्गीकृत किया जा सकता है।
    (a) एकदंतुर एक दाता परमाणु, उदाहरण $\mathrm{NH}_{3}, \bullet \mathrm{Cl}$ आदि।
    (b) द्विदंतुर दो दाता - परमाणु, उदाहरण $\mathrm{H}_{2} \mathrm{NCH}_{2} \mathrm{CH}_{2} \mathrm{NH}_{2}$ (एथेन-1 . 2-डाइऐमीन) तथा $\mathrm{C}_{2} \mathrm{O}_{4}^{2-}$ (ऑक्सेलेट आयन आदि)।
    (c) बहुदंतुर दो से अधिक दाता परमाणु, उदाहरण $\mathrm{N}\left(\mathrm{CH}_{2} \mathrm{CH}_{2} \mathrm{NH}_{2}\right)_{3}$, EDTA आदि।
(iii) उपसहसंयोजन संख्या एक संकुल में धातु आयन की उपसहसंयोजन संख्या (CN) उससे आबंधित लिगेण्डों के उन दाता परमाणुओं की संख्या के बराबर होती है जो सीधे धातु आयन से जुड़े हों। उदाहरण संकुल आयनों $\left[\mathrm{PtCl}_{6}\right]^{2-}$ तथा $\left[\mathrm{Ni}\left(\mathrm{NH}_{3}\right)_{4}\right]^{2+}$ में Pt तथा Ni की उपसहसंयोजन संख्या क्रमशः $6$ तथा $4$ है।

(iv) उपसहसंयोजन बहुफलक केन्द्रीय परमाणु /आयन से सीधे-जुड़े लिगेण्ड परमाणुओं की दिक्स्थान व्यवस्था को उपसहसंयोजन बहुफलक कहते हैं। इनमें अष्टफलकीय, वर्ग समतलीय तथा चतुष्फलकीय मुख्य हैं।
उदाहरण $\left[\mathrm{Co}\left(\mathrm{NH}_{3}\right)_{6}\right]^{3+}$ अष्टफलकीय है, $\left[\mathrm{Ni}(\mathrm{CO})_{4}\right]$ चतुष्फलकीय है तथा
$\left[\mathrm{PtCl}_{4}\right]^{2-}$ वर्ग समतली है।


जहाँ $M$ धातु परमाणु तथा $L$ लिगेण्डों को दर्शाता है।

(v) होमोलेप्टिक संकुल इन संकुलों में धातु परमाणु केवल एक प्रकार के दाता समूह से जुड़ा रहता है। उदाहरण $\left[\mathrm{Co}\left(\mathrm{NH}_{3}\right)_{6}\right]^{3+}$


(vi) हेट्रोलेप्टिक संकुल इन संकुलों में धातु परमाणु एक से अधिक प्रकार के दाता समूहों से जुड़ा रहता है। उदाहरण, $\left[\mathrm{Co}\left(\mathrm{NH}_{3}\right)_{4} \mathrm{Cl}_{2}\right]$
:::

:::

:::question{number="5.4" kind="exercise" id="q_5.4" topic="Unidentate, bidentate, ambidentate ligands"}
#### प्रश्न 5.4

:::prompt
एकदंतुर, द्विदंतुर तथा उभयदंतुर लिगन्ड से क्या तात्पर्य है? प्रत्येक के दो उदाहरण दीजिए।
:::

:::figure{src="images/fig_5_95.svg" id="fig_5_95"}
:::

:::figure{src="images/fig_5_96.svg" id="fig_5_96"}
:::

:::figure{src="images/fig_5_97.svg" id="fig_5_97"}
एथिलीन डाइऐमीन (en)
:::

:::figure{src="images/fig_5_98.svg" id="fig_5_98"}
नाइट्राइटों-N
:::

:::solution{label="हल"}
(i) एकदंतुर एक दाता परमाणु रखने वाले ऋणायन/उदासीन अणु एकदंतुर लिगेण्ड कहलाते हैं।
उदाहरण $\mathrm{NH}_{3}, \mathrm{Cl}^{-}, \mathrm{H}_{2} \mathrm{O}$ आदि।

    (ii) द्विदंतुर दो दाता परमाणु रखने वाले आयन या अणु, द्विदंतुर लिगेण्ड कहलाते हैं।
उदाहरण ऑक्सेलेट आयन; एथिलीन डाइऐमीन (en)

    (iii) उमय दंतुर जो लिगेण्ड दो भिन्न परमाणुओं द्वारा जुड़ सकता है उसे उभयदंती संलग्नी या उमयदंती लिगेण्ड कहते हैं उदाहरण $\mathrm{NO}_{2}^{-}, \mathrm{SCN}^{-}$आयन
$\mathrm{NO}_{2}^{-}$आयन केन्द्रीय धातु परमाणु/आयन से या तो नाइट्रोजन द्वारा अथवा ऑक्सीजन द्वारा संयोजित हो सकता है। इसी प्रकार, $\mathrm{SCN}^{-}$आयन सल्फर अथवा नाइट्रोजन परमाणु द्वारा संयोजित हो सकता है।

$M \leftarrow \mathrm{N}(=\mathrm{O})(=\mathrm{O})$ नाइट्राइटो-N $\left(\mathrm{NO}_{2}^{-}\right)$
$M \leftarrow \mathrm{O}-\mathrm{N}=\mathrm{O}$ नाइट्राइटो-O $\left(\mathrm{NO}_{2}^{-}\right)$
$M \leftarrow \mathrm{SCN}$ थायोसायनेटो $\left(\mathrm{SCN}^{-}\right)$
$M \leftarrow \mathrm{NCS}$ आइसो थायोसायनेटो $\left(\mathrm{SCN}^{-}\right)$
(जहाँ, $M=$ धातु परमाणु)
:::

:::

:::question{number="5.5" kind="exercise" id="q_5.5" topic="Oxidation number of metals in coordination entities" corrections_applied="1"}
#### प्रश्न 5.5

:::prompt
निम्नलिखित उपसहसंयोजन सत्ता में धातुओं के ऑक्सीकरण अंक का उल्लेख कीजिए-
:::

:::part{label="(i)"}
:::prompt
$\left[\mathrm{Co}\left(\mathrm{H}_{2} \mathrm{O}\right)(\mathrm{CN})(\text { en })_{2}\right]^{2+}$
:::

:::

:::part{label="(ii)"}
:::prompt
$\left[\mathrm{CoBr}_{2}(\mathrm{en})_{2}\right]^{+}$
:::

:::

:::part{label="(iii)"}
:::prompt
$\left[\mathrm{PtCl}_{4}\right]^{2-}$
:::

:::

:::part{label="(iv)"}
:::prompt
$\mathrm{K}_{3}\left[\mathrm{Fe}(\mathrm{CN})_{6}\right]$
:::

:::

:::part{label="(v)"}
:::prompt
$\left[\mathrm{Cr}\left(\mathrm{NH}_{3}\right)_{3} \mathrm{Cl}_{3}\right]$
:::

:::

:::solution{label="हल"}
माना कि धातु की ऑक्सीकरण संख्या $x$ है।

(i) $\left[\mathrm{Co}\left(\mathrm{H}_{2} \mathrm{O}\right)(\mathrm{CN})(\text { en })_{2}\right]^{2+}$
$$
\begin{aligned}
x+0+(-1)+(2 \times 0) & =+2 \\
x & =2+1=+3
\end{aligned}
$$
(ii) $\left[\mathrm{CoBr}_{2}\left(\mathrm{en}_{2}\right]^{+}\right.$
$$
\begin{aligned}
x+2 \times(-1)+(2 \times 0) & =+1 \\
x & =1+2=+3
\end{aligned}
$$
(iii) $\left[\mathrm{PtCl}_{4}\right]^{2-}$
$$
\begin{aligned}
x+(-1) 4 & =-2 \\
x-4 & =-2 \\
x & =-2+4=+2
\end{aligned}
$$
(iv) $\mathbf{K}_{\mathbf{3}} \mathbf{F e}(\mathbf{C N})_{\mathbf{6}}$
$$
\begin{aligned}
(+1) \times 3+x+(-1) \times 6 & =0 \\
3+x-6 & =0 \\
x-3 & =0 \\
x & =+3
\end{aligned}
$$
(v) $\left[\mathrm{Cr}\left(\mathrm{NH}_{3}\right)_{3} \mathrm{Cl}_{3}\right]$
$$
\begin{aligned}
x+(3 \times 0)+(3 \times(-1)) & =0 \\
x & =+3
\end{aligned}
$$
:::

:::answer
**उत्तर:** (i) $+3$
(ii) $+3$
(iii) $+2$
(iv) $+3$
(v) $+3$
:::

:::

:::question{number="5.6" kind="exercise" id="q_5.6" topic="IUPAC formulas from names"}
#### प्रश्न 5.6

:::prompt
IUPAC नियमों के आधार पर निम्नलिखित के लिये सूत्र लिखिए-
:::

:::part{label="(i)"}
:::prompt
टेट्राहाइड्रॉक्सिडोजिंकेट(II)
:::

:::

:::part{label="(ii)"}
:::prompt
पोटैशियम टेट्राक्लोरिडोपैलेडेट(II)
:::

:::

:::part{label="(iii)"}
:::prompt
डाइऐम्मीनडाइक्लोरिडो प्लेटिनम(II)
:::

:::

:::part{label="(iv)"}
:::prompt
पोटैशियम टेट्रासायनिडोनिकैलेट(II)
:::

:::

:::part{label="(v)"}
:::prompt
पेन्टाऐम्मीननाइट्रिटो-O-कोबाल्ट(III)
:::

:::

:::part{label="(vi)"}
:::prompt
हेक्साऐम्मीनकोबाल्ट(III)सल्फेट
:::

:::

:::part{label="(vii)"}
:::prompt
पोटैशियम ट्राइआक्सैलेटोक्रोमेट(III)
:::

:::

:::part{label="(viii)"}
:::prompt
हेक्साऐम्मीनप्लैटिनम(IV)
:::

:::

:::part{label="(ix)"}
:::prompt
टेट्राब्रोमिडो क्यूप्रेट(II)
:::

:::

:::part{label="(x)"}
:::prompt
पेन्टाऐम्मीननाइट्रिटो-N-कोबाल्ट(III)
:::

:::

:::solution{label="हल"}
(i) $\left[\mathrm{Zn}(\mathrm{OH})_{4}\right]^{2-}$
(ii) $\mathrm{K}_{2}\left[\mathrm{PdCl}_{4}\right]$
(iii) $\left[\mathrm{Pt}\left(\mathrm{NH}_{3}\right)_{2} \mathrm{Cl}_{2}\right]$
(iv) $\mathrm{K}_{2}\left[\mathrm{Ni}(\mathrm{CN})_{4}\right]$

(v) $\left[\mathrm{Co}\left(\mathrm{NH}_{3}\right)_{5}(\mathrm{ONO})\right]^{2+}$

(O दर्शाता है कि $\mathrm{NO}_{2}$ समूह घातु परमाणु से ऑक्सीजन के द्वारा जुड़ा है।)
(vi) $\left[\mathrm{Co}\left(\mathrm{NH}_{3}\right)_{6}\right]_{2}\left(\mathrm{SO}_{4}\right)_{3}$
(vii) $\mathrm{K}_{3}\left[\mathrm{Cr}\left(\mathrm{C}_{2} \mathrm{O}_{4}\right)_{3}\right]$
(viii) $\left[\mathrm{Pt}\left(\mathrm{NH}_{3}\right)_{6}\right]^{4+}$
(ix) $\left[\mathrm{CuBr}_{4}\right]^{2-}$
(x) $\left[\mathrm{Co}\left(\mathrm{NH}_{3}\right)_{5}\left(\mathrm{NO}_{2}\right)\right]^{2+}$
:::

:::

:::question{number="5.7" kind="exercise" id="q_5.7" topic="IUPAC names of coordination compounds" corrections_applied="1"}
#### प्रश्न 5.7

:::prompt
IUPAC नियमों के आधार पर निम्नलिखित के सुव्यवस्थित नाम लिखिए-
:::

:::part{label="(i)"}
:::prompt
$\left[\mathrm{Co}\left(\mathrm{NH}_{3}\right)_{6}\right] \mathrm{Cl}_{3}$
:::

:::

:::part{label="(ii)"}
:::prompt
$\left[\mathrm{Pt}\left(\mathrm{NH}_{3}\right)_{2} \mathrm{Cl}\left(\mathrm{NH}_{2} \mathrm{CH}_{3}\right)\right] \mathrm{Cl}$
:::

:::

:::part{label="(iii)"}
:::prompt
$\left[\mathrm{Ti}\left(\mathrm{H}_{2} \mathrm{O}\right)_{6}\right]^{3+}$
:::

:::

:::part{label="(iv)"}
:::prompt
$\left[\mathrm{Co}\left(\mathrm{NH}_{3}\right)_{4} \mathrm{Cl}\left(\mathrm{NO}_{2}\right)\right] \mathrm{Cl}$
:::

:::

:::part{label="(v)"}
:::prompt
$\left[\mathrm{Mn}\left(\mathrm{H}_{2} \mathrm{O}\right)_{6}\right]^{2+}$
:::

:::

:::part{label="(vi)"}
:::prompt
$\left[\mathrm{NiCl}_{4}\right]^{2-}$
:::

:::

:::part{label="(vii)"}
:::prompt
$\left[\mathrm{Ni}\left(\mathrm{NH}_{3}\right)_{6}\right] \mathrm{Cl}_{2}$
:::

:::

:::part{label="(viii)"}
:::prompt
$\left[\mathrm{Co}(\mathrm{en})_{3}\right]^{3+}$
:::

:::

:::part{label="(ix)"}
:::prompt
$\left[\mathrm{Ni}(\mathrm{CO})_{4}\right]$
:::

:::

:::solution{label="हल"}
(i) हेक्साऐमीनकोबाल्ट (III) क्लोराइड
(ii) डाइऐमीनक्लोरोडो (मिथाइल ऐमीन) प्लेटिनम (II) क्लोराइड
(iii) हेक्साएक्वाटाइटेनियम (III) आयन
(iv) टेट्राऐमीनक्लोरोडोनाइट्राइटो-N-कोबाल्ट (III) क्लोराइड
(v) हेक्साएक्वामैंगनीज (II) आयन
(vi) टेट्राक्लोरोडोनिकैलेट (II) आयन
(vii) हेक्साऐमीननिकैल (II) क्लोराइड
(viii) ट्रिस(एथेन-1, 2-डाइऐमीन) कोबाल्ट (III) आयन
(ix) टेट्राकार्बोनिल निकैल (0)
:::

:::

:::question{number="5.8" kind="exercise" id="q_5.8" topic="Types of isomerism in coordination compounds" corrections_applied="4"}
#### प्रश्न 5.8

:::prompt
उपसहसंयोजन यौगिकों के लिए संभावित विभिन्न प्रकार की समावयवताओं को सूचीबद्ध कीजिए तथा प्रत्येक का एक उदाहरण दीजिए।
:::

:::figure{src="images/fig_5_51.jpg" id="fig_5_51"}
:::

:::figure{src="images/fig_5_52.jpg" id="fig_5_52"}
दक्षिणावर्त ( $d$-)
:::

:::figure{src="images/fig_5_53.jpg" id="fig_5_53"}
वामावर्त ( $l$-)
:::

:::solution{label="हल"}
उपसहसंयोजक यौगिकों में समावयवता मुख्यतः दो प्रकार की होती है।

1. संरचनात्मक समावयवता
(a) बंधनी समावयवता
(b) उपसहसंयोजन समावयवता
(c) आयनन समावयवता
(d) विलायकयोजन समावयवता
2. त्रिविम समावयवता
(a) ज्यामितीय समावयवता
(b) ध्रुवण समावयवता

उदाहरण

1. (a) बंधनी समावयवता
    (i) $\left[\mathrm{Co}\left(\mathrm{NH}_{3}\right)_{5} \mathrm{NO}_{2}\right] \mathrm{Cl}_{2}$ तथा $\left[\mathrm{Co}\left(\mathrm{NH}_{3}\right)_{5} \mathrm{ONO}\right] \mathrm{Cl}_{2}$
    (ii) $\left[\mathrm{Mn}(\mathrm{CO})_{5} \mathrm{SCN}\right]$ तथा $\left[\mathrm{Mn}(\mathrm{CO})_{5} \mathrm{NCS}\right]$
    (b) उपसहसंयोजन समावयवता
    (i) $\left[\mathrm{Co}\left(\mathrm{NH}_{3}\right)_{6}\right]\left[\mathrm{Cr}(\mathrm{CN})_{6}\right]$ तथा $\left[\mathrm{Cr}\left(\mathrm{NH}_{3}\right)_{6}\right]\left[\mathrm{Co}(\mathrm{CN})_{6}\right]$
    (c) आयनन समावयवता
        (i) $\left[\mathrm{Co}\left(\mathrm{NH}_{3}\right)_{5} \mathrm{SO}_{4}\right] \mathrm{Br}$ तथा $\left[\mathrm{Co}\left(\mathrm{NH}_{3}\right)_{5} \mathrm{Br}\right] \mathrm{SO}_{4}$
    (d) विलायकयोजन समावयवता
    (i) $\left[\mathrm{Cr}\left(\mathrm{H}_{2} \mathrm{O}\right)_{6}\right] \mathrm{Cl}_{3}$ (बैंगनी) तथा $\left[\mathrm{Cr}\left(\mathrm{H}_{2} \mathrm{O}\right)_{5} \mathrm{Cl}\right] \mathrm{Cl}_{2} \cdot \mathrm{H}_{2} \mathrm{O}$ (स्लेटी हरा)
2. (a) ज्यामितीय समावयवता
    (i) $\left[\mathrm{CoCl}_{2}(\mathrm{en})_{2}\right]$ के समपक्ष तथा विपक्ष समावयव

    (b)ध्रुवण समावयवता
(i) $\left[\mathrm{Co}(\mathrm{en})_{3}\right]^{3+}$ के दो रूप दक्षिणावर्त ($d$) और वामावर्त ($l$) हैं।

विपक्ष रूप


दर्पण
:::

:::

:::question{number="5.9" kind="exercise" id="q_5.9" topic="Number of geometrical isomers possible"}
#### प्रश्न 5.9

:::prompt
निम्नलिखित उपसहसंयोजन सत्ता में कितने ज्यामितीय समावयव संभव हैं?
:::

:::figure{src="images/fig_5_54.jpg" id="fig_5_54"}
:::

:::part{label="(क)"}
:::prompt
$\left[\mathrm{Cr}\left(\mathrm{C}_{2} \mathrm{O}_{4}\right)_{3}\right]^{3-}$
:::

:::

:::part{label="(ख)"}
:::prompt
$\left[\mathrm{Co}\left(\mathrm{NH}_{3}\right)_{3} \mathrm{Cl}_{3}\right]$
:::

:::

:::solution{label="हल"}
(i) ज्यामितीय समावयव संभव नहीं
(ii) $\left[\mathrm{Co}\left(\mathrm{NH}_{3}\right)_{3} \mathrm{Cl}_{3}\right]$ के लिए दो ज्यामितीय समावयव संभव हैं।
:::

:::answer
**उत्तर:** (i) ज्यामितीय समावयव संभव नहीं
(ii) दो ज्यामितीय समावयव संभव हैं
:::

:::

:::question{number="5.10" kind="exercise" id="q_5.10" topic="Structures of optical isomers" corrections_applied="2"}
#### प्रश्न 5.10

:::prompt
निम्न के प्रकाशित समावयवों की संरचनाएं बनाइए-
:::

:::figure{src="images/fig_5_55.jpg" id="fig_5_55"}
:::

:::figure{src="images/fig_5_56.jpg" id="fig_5_56"}
:::

:::figure{src="images/fig_5_57.jpg" id="fig_5_57"}
:::

:::part{label="(i)"}
:::prompt
$\left[\mathrm{Cr}\left(\mathrm{C}_{2} \mathrm{O}_{4}\right)_{3}\right]^{3-}$
:::

:::

:::part{label="(ii)"}
:::prompt
$\left[\mathrm{PtCl}_{2}(\mathrm{en})_{2}\right]^{2+}$
:::

:::

:::part{label="(iii)"}
:::prompt
$\left[\mathrm{Cr}\left(\mathrm{NH}_{3}\right)_{2} \mathrm{Cl}_{2}(\mathrm{en})\right]^{+}$
:::

:::

:::solution{label="हल"}
(i) $\left[\mathrm{Cr}\left(\mathrm{C}_{2} \mathrm{O}_{4}\right)_{3}\right]^{3-}$

(ii) $\mathbf{C i s}-\left[\mathbf{P t C l}_{\mathbf{2}}\left(\mathbf{e n}\right)_{\mathbf{2}}\right]^{\mathbf{2}+}$


(iii) $\mathbf{C i s}-\left[\mathbf{C r}\left(\mathbf{N H}_{\mathbf{3}}\right)_{\mathbf{2}} \mathbf{C l}_{\mathbf{2}}(\mathbf{e n})\right]^{+}$
:::

:::

:::question{number="5.11" kind="exercise" id="q_5.11" topic="All geometrical and optical isomer structures"}
#### प्रश्न 5.11

:::prompt
निम्नलिखित के सभी समायवों (ज्यामितीय व ध्रुवण) की संरचनाएं बनाइए-
:::

:::figure{src="images/fig_5_58.jpg" id="fig_5_58"}
:::

:::figure{src="images/fig_5_59.jpg" id="fig_5_59"}
:::

:::part{label="(i)"}
:::prompt
$\left[\mathrm{CoCl}_{2}(\mathrm{en})_{2}\right]^{+}$
:::

:::

:::part{label="(ii)"}
:::prompt
$\left[\mathrm{Co}\left(\mathrm{NH}_{3}\right) \mathrm{Cl}(\text { en })_{2}\right]^{2+}$
:::

:::

:::part{label="(iii)"}
:::prompt
$\left[\mathrm{Co}\left(\mathrm{NH}_{3}\right)_{2} \mathrm{Cl}_{2}(\text { en })\right]^{+}$
:::

:::

:::solution{label="हल"}
(i) $\left[\mathrm{CoCl}_{2}(\mathrm{en})_{2}\right]^{+}$इस आयन के दो ज्यामितीय समावयव हैं। समपक्ष रूप ध्रुवण समावयवता भी दर्शाता है।
:::

:::

:::question{number="5.12" kind="exercise" id="q_5.12" topic="Geometrical isomers of a Pt complex"}
#### प्रश्न 5.12

:::prompt
$\left[\mathrm{Pt}\left(\mathrm{NH}_{3}\right)(\mathrm{Br})(\mathrm{Cl})(\mathrm{py})\right]$ के सभी ज्यामितीय समावयव लिखिए। इनमें से कितने ध्रुवण समावयवता दर्शाएंगे?
:::

:::figure{src="images/fig_5_60.jpg" id="fig_5_60"}
[I]
:::

:::figure{src="images/fig_5_61.jpg" id="fig_5_61"}
[II]
:::

:::figure{src="images/fig_5_62.jpg" id="fig_5_62"}
[III]
:::

:::solution{label="हल"}
$\left[\mathrm{Pt}\left(\mathrm{NH}_{3}\right)(\mathrm{Br})(\mathrm{Cl})(\mathrm{py})\right]$ इस संकुल के लिए तीन ज्यामितीय समावयवी संभव हैं


यह यौगिक $\mathrm{CN}=4$ तथा वर्गसमतली ज्यामिति के साथ धुवण समावयवता नहीं दर्शाता है क्योंकि इसमें सममिति तल उपस्थित है।
:::

:::answer
**उत्तर:** तीन ज्यामितीय समावयवी संभव हैं। यह यौगिक ध्रुवण समावयवता नहीं दर्शाता है क्योंकि इसमें सममिति तल उपस्थित है।
:::

:::

:::question{number="5.13" kind="exercise" id="q_5.13" topic="Colour changes of aqueous copper sulfate" corrections_applied="1"}
#### प्रश्न 5.13

:::prompt
जलीय कॉपर सल्फेट विलयन (नीले रंग का), निम्नलिखित प्रेक्षण दर्शाता है-


उपरोक्त प्रायोगिक परिणामों को समझाइए।
:::

:::part{label="(i)"}
:::prompt
जलीय पोटैशियम फ्लुओराइड के साथ हरा रंग
:::

:::

:::part{label="(ii)"}
:::prompt
जलीय पोटैशियम क्लोराइड के साथ चमकीला हरा रंग
:::

:::

:::solution{label="हल"}
जलीय कॉपर सल्फेट (नीला) $\left[\mathrm{Cu}\left(\mathrm{H}_{2} \mathrm{O}\right)_{4}\right] \mathrm{SO}_{4}$ है।

$$
\left[\mathrm{Cu}\left(\mathrm{H}_{2} \mathrm{O}\right)_{4}\right] \mathrm{SO}_{4} \stackrel{a q}{\rightleftharpoons} \underset{[\text { नीला }]}{\left[\mathrm{Cu}\left(\mathrm{H}_{2} \mathrm{O}\right)_{4}\right]^{2+}}+\mathrm{SO}_{4}^{2-}
$$

$\left[\mathrm{Cu}\left(\mathrm{H}_{2} \mathrm{O}\right)_{4}\right]^{2+}$ एक ऐसा संकुल है जिसमें दुर्बल $\mathrm{H}_{2} \mathrm{O}$ लिगेण्ड $\mathrm{F}^{-}$लिगेण्डों द्वारा प्रतिस्थापित होकर $\left[\mathrm{CuF}_{4}\right]^{2+}$ (हरा) संकुल आयन देते हैं तथा $\mathrm{Cl}^{-}$लिगेण्डों द्वारा प्रतिस्थापित होकर संकुल आयन $\left[\mathrm{CuCl}_{4}\right]^{2-}$ (चमकीला हरा) देते हैं।

(i) $\left[\mathrm{Cu}\left(\mathrm{H}_{2} \mathrm{O}\right)_{4}\right]^{2+}(\mathrm{aq})+4 \mathrm{~F}^{-}(\mathrm{aq}) \longrightarrow \underset{\text { हरा अवक्षेप }}{\left[\mathrm{CuF}_{4}\right]^{2-}}+4 \mathrm{H}_{2} \mathrm{O}$
(ii) $\left[\mathrm{Cu}\left(\mathrm{H}_{2} \mathrm{O}\right)_{4}\right]^{2+}(\mathrm{aq})+4 \mathrm{Cl}^{-}(\mathrm{aq}) \longrightarrow \underset{\text { चमकीला हरा विलयन }}{\left[\mathrm{CuCl}_{4}\right]^{2-}(\mathrm{aq})}+4 \mathrm{H}_{2} \mathrm{O}$
:::

:::

:::question{number="5.14" kind="exercise" id="q_5.14" topic="Copper-cyanide complex and absence of CuS precipitate" corrections_applied="1"}
#### प्रश्न 5.14

:::prompt
कॉपर सल्फेट के जलीय विलयन में जलीय KCN को आधिक्य में मिलाने पर बनने वाली उपसहसंयोजन सत्ता क्या होगी? इस विलयन में जब $\mathrm{H}_{2} \mathrm{~S}$ गैस प्रवाहित की जाती है तो कॉपर सल्फाइड का अवक्षेप क्यों नहीं प्राप्त होता?
:::

:::solution{label="हल"}
जब KCN (aq) के आधिक्य में $\mathrm{CuSO}_{4}(\mathrm{aq})$ मिलाते हैं तो पोटैशियम टेट्रासायनो क्यूपरेट (II) बनता है। $\mathrm{CN}^{-}$आयन प्रबल क्षेत्र लिगेण्ड हैं अतः प्राप्त संकुल पर्याप्त स्थाई होता है। स्थायित्व स्थिरांक का मान $\left(K=2.0 \times 10^{27}\right)$ भी इसकी पुष्टि करता है।

$$
\begin{gathered}
\underset{\text { (आधिक्य) }}{4 \mathrm{KCN}(a q)}+\mathrm{CuSO}_{4}(a q) \longrightarrow \underset{\text { (विलेय) }}{\mathrm{K}_{2}\left[\mathrm{Cu}(\mathrm{CN})_{4}\right](a q)+\mathrm{K}_{2} \mathrm{SO}_{4}(a q)} \\
\downarrow+\mathrm{H}_{2} \mathrm{~S}
\end{gathered}
$$

कोई वियोजन नहीं अतः $\mathrm{Cu}^{2+}$ आयन उत्पन्न नहीं होते
:::

:::

:::question{number="5.15" kind="exercise" id="q_5.15" topic="Nature of bonding by valence bond theory" corrections_applied="6"}
#### प्रश्न 5.15

:::prompt
संयोजकता आबंध सिद्धांत के आधार पर निम्नलिखित उपसहसंयोजन सत्ता में आबंध की प्रकृति की विवेचना कीजिए-
:::

:::figure{src="images/fig_5_63.jpg" id="fig_5_63"}
:::

:::figure{src="images/fig_5_64.jpg" id="fig_5_64"}
:::

:::figure{src="images/fig_5_65.jpg" id="fig_5_65"}
:::

:::figure{src="images/fig_5_66.jpg" id="fig_5_66"}
:::

:::figure{src="images/fig_5_67.jpg" id="fig_5_67"}
:::

:::figure{src="images/fig_5_68.jpg" id="fig_5_68"}
:::

:::part{label="(क)"}
:::prompt
$\left[\mathrm{Fe}(\mathrm{CN})_{6}\right]^{4-}$
:::

:::

:::part{label="(ख)"}
:::prompt
$\left[\mathrm{FeF}_{6}\right]^{3-}$
:::

:::

:::part{label="(ग)"}
:::prompt
$\left[\mathrm{Co}\left(\mathrm{C}_{2} \mathrm{O}_{4}\right)_{3}\right]^{3-}$
:::

:::

:::part{label="(घ)"}
:::prompt
$\left[\mathrm{CoF}_{6}\right]^{3-}$
:::

:::

:::solution{label="हल"}
(i) $\left[\mathrm{Fe}(\mathrm{CN})_{6}\right]^{4-}$ इस संकुल में $\mathrm{Fe}, \mathrm{Fe}^{2+}$ रूप में है

$$
\mathrm{Fe}=[\mathrm{Ar}] 3 d^{6} 4 s^{2}
$$

$\mathrm{Fe}^{2+}$ का बाहरी विन्यास $=3 d^{6} 4 \mathrm{~s}^{0}$

$$
\mathrm{Fe}^{2+}=\begin{array}{|l|l|l|l|l|}
\hline ് ബ & \frac{3 d^{6}}{4 s^{0}} & & & \\
\hline
\end{array} \begin{array}{|l|l|l|}
\hline 4 \rho^{0} \\
\hline
\end{array}
$$

$\mathrm{CN}^{-}$प्रबल क्षेत्र लिगेण्ड होने के कारण अयुग्मित $d$-इलेक्ट्रॉनों को युग्मित कर देता है। इस प्रकार $\mathrm{CN}^{-}$आयनों को दो $3 d$-कक्षक उपलब्ध हो जाती हैं।

$$
\left[\mathrm{Fe}(\mathrm{CN})_{6}\right]^{4-}=\underbrace{\begin{array}{|l|l|l|l|}
\hline 1 & 1 l & 1 l & \times x \\
\hline
\end{array}}_{\substack{\text { कोई अयुग्मित } \\
\text { इलेक्ट्रॉन नहीं }}} \underbrace{\begin{array}{|l|}
\hline \times- \\
\hline
\end{array}}_{d^{2} s p^{3} \text { संकरण }} \underbrace{\begin{array}{|l|l|}
\hline 4 s & \\
\hline
\end{array}}_{\begin{array}{|l|}
\hline \times x \\
\hline
\end{array}}
$$

क्योंकि सभी इलेक्ट्रॉन युग्मित हैं। अतः यह संकुल प्रतिचुम्बकीय होगा तथा आबंधन में $(n-1) d$-कक्षक भाग लेने के कारण, यह आन्तरिक कक्षक तथा निम्न प्रचक्रण संकुल होगा।

(ii) $\left[\mathrm{Fe} \mathrm{F}_{6}\right]^{3-}$ इस संकुल में Fe की ऑक्सीकरण अवस्था $+3$ है।
$$
\mathrm{Fe}^{3+}=3 d^{5} 4 s^{0}
$$

$\mathrm{F}^{-}$प्रबल क्षेत्र लिगेण्ड नहीं है यह एक दुर्बल क्षेत्र लिगेण्ड है। अतः इलेक्ट्रॉनों का युग्मन नहीं होगा। इस प्रकार $d$-कक्षक आबंधन के लिए उपलब्ध नहीं है।

क्योंकि पाँच अयुग्मित इलेक्ट्रॉन उपस्थित हैं, अत: यह संकुल अनुचम्बकीय है इसके आबंधन में $n d$-कक्षक के भाग लेने के कारण यह बाह्य कक्षक तथा उच्च प्रचक्रण संकुल है।

(iii) $\left[\mathrm{Co}\left(\mathrm{C}_{2} \mathrm{O}_{4}\right)_{3}\right]^{3-}$ इस संकुल में Co की ऑक्सीकरण अवस्था $+3$ है।
Co का बाहरी विन्यास $=3 d^{7} 4 s^{2}$
$$
\mathrm{Co}^{3+}=3 d^{6} 4 s^{0}
$$

ऑक्सेलेट आयन प्रबल क्षेत्र लिगेण्ड होने के कारण $3 d$-इलेक्ट्रॉनों को युग्मित कर देता है इस प्रकार पाँच में से दो $3 d$-कक्षक ऑक्सेलेट आयनों के लिए उपलब्ध हैं।

क्योंकि सभी इलेक्ट्रॉन युग्मित हैं अत: यह संकुल प्रतिचुम्बकीय होगा। यह आन्तरिक कक्षक संकुल है क्योंकि इसके आबंधन में $(n-1) d$-कक्षक भाग लेते है।

(iv) $\left[\mathrm{CoF}_{6}\right]^{3-}$ इस संकुल में Co की ऑक्सीकरण अवस्था $+3$ है।
Co का बाहरी विन्यास $=3 d^{6} 4 s^{0}$

$\mathrm{F}^{-}$एक दुर्बल क्षेत्र लिगेण्ड हैं। अतः इलेक्ट्रॉनों का युग्मन नहीं होगा और इस प्रकार $F^{-}$के लिए $n d$-कक्षक उपलब्ध है।

चार अयुग्मित इलेक्ट्रॉनों के कारण संकुल अनुचुम्बकीय है। क्योंकि $n d$-कक्षक आबंधन में भाग ले रही हैं अत: यह बाहरी कक्षक संकुल व उच्च प्रचक्रण संकुल है।
:::

:::

:::question{number="5.16" kind="exercise" id="q_5.16" topic="d-orbital splitting diagram in octahedral field"}
#### प्रश्न 5.16

:::prompt
अष्टफलकीय क्रिस्टल क्षेत्र में $d$ कक्षकों के विपाटन को दर्शाने के लिए चित्र बनाइए।
:::

:::figure{src="images/fig_5_69.jpg" id="fig_5_69"}
अष्टफलकीय क्रिस्टल क्षेत्र में $d$ कक्षकों का विपाटन
:::

:::

:::question{number="5.17" kind="exercise" id="q_5.17" topic="Spectrochemical series" corrections_applied="4"}
#### प्रश्न 5.17

:::prompt
स्पेक्ट्रमीरासायनिक श्रेणी क्या है? दुर्बल क्षेत्र लिगन्ड तथा प्रबल क्षेत्र लिगन्ड में अंतर स्पष्ट कीजिए।
:::

:::solution{label="हल"}
हल लिगेण्डों को उनकी बढ़ती हुई क्षेत्र प्रबलता के क्रम में एक श्रेणी में व्यवस्थित किया जाता है अर्थात् क्रिस्टल क्षेत्र विपाटन ऊर्जा मानों के बढ़ते क्रम में व्यवस्थित करने पर प्राप्त श्रेणी को स्पेक्ट्रमी रासायनिक श्रेणी कहते हैं।

$$
\begin{aligned}
\mathrm{I}^{-}<\mathrm{Br}^{-}<\mathrm{SCN}^{-} & <\mathrm{Cl}^{-}<\mathrm{F}^{-}<\mathrm{OH}^{-}<\mathrm{ox}<\mathrm{H}_{2} \mathrm{O} \\
& <\mathrm{EDTA}^{4-}<\mathrm{NH}_{3}<\mathrm{en}<\mathrm{CN}^{-}<\mathrm{CO}
\end{aligned}
$$

स्पेक्ट्रमी रासायनिक श्रेणी
निम्न क्रिस्टल क्षेत्र विपाटन ऊर्जा CFSE $\left(\Delta_{0}\right)$ मान वाले लिगेण्ड दुर्बल क्षेत्र लिगेण्ड हैं इस प्रकार के लिगेण्डों के लिए $\Delta_{0}<P$, जहाँ $P$ युग्मन ऊर्जा है, जबकि उच्च $\operatorname{CFSE}\left(\Delta_{0}\right)$ मानों वाले लिगेण्ड प्रबल क्षेत्र लिगेण्ड हैं इनके लिए $\Delta_{0}>P$
:::

:::

:::question{number="5.18" kind="exercise" id="q_5.18" topic="Crystal field splitting energy" corrections_applied="1"}
#### प्रश्न 5.18

:::prompt
क्रिस्टल क्षेत्र विपाटन ऊर्जा क्या है? उपसहसंयोजन सत्ता में $d$ कक्षकों का वास्तविक विन्यास $\Delta_{o}$ के मान के आधार पर कैसे निर्धारित किया जाता है?
:::

:::solution{label="हल"}
हल जब लिगेण्ड एक घातु आयन की ओर जाता है तो $d$-कक्षकों का विपाटन दो समुच्चयों ( $t_{2 g}$ तथा $e_{g}$ ) में हो जाता है। $t_{2 g}$ निम्न ऊर्जा वाला तथा $e_{g}$ उच्च ऊर्जा वाला समुच्चय है। दोनों

समुच्चय के कक्षकों के बीच ऊर्जा के अन्तर को क्रिस्टल क्षेत्र विपाटन ऊर्जा ( $\Delta_{0}$ ) कहते हैं। ( $\Delta_{0}=$ अष्टफलकीय क्षेत्र तथा $\Delta_{t}=$ चतुष्फलकीय क्षेत्र के लिए)
यदि $\Delta_{0}<P$ (युग्मन ऊर्जा), तो चौथा इलेक्ट्रॉन किसी एक $e_{g}$ कक्षक में प्रवेश करेगा तथा विन्यास $t_{2 g}^{3} e_{g}^{1}$ होगा अर्थात् उच्च प्रचक्रण संकुल बनेगा। इस प्रकार के लिगेण्ड जिनके लिए $\Delta_{0}<P$ है दुर्बल क्षेत्र लिगेण्ड कहलाते हैं।
यदि $\Delta_{0}>P$, तो चौथा इलेक्ट्रॉन $t_{2 g}$ कक्षकों में से किसी एक कक्षक में प्रवेश करेगा अर्थात् $t_{2 g}^{4} e_{g}^{0}$ विन्यास के साथ निम्न क्षेत्र संकुल बनेंगे। ऐसे लिगेण्ड जिनके लिए $\Delta_{0}>P$ है प्रबल क्षेत्र लिगेण्ड कहलाते हैं।
:::

:::

:::question{number="5.19" kind="exercise" id="q_5.19" topic="Paramagnetism vs diamagnetism explanation" corrections_applied="2"}
#### प्रश्न 5.19

:::prompt
$\left[\mathrm{Cr}\left(\mathrm{NH}_{3}\right)_{6}\right]^{3+}$ अनुचुंबकीय है जबकि $\left[\mathrm{Ni}(\mathrm{CN})_{4}\right]^{2-}$ प्रतिचुंबकीय, समझाइए क्यों?
:::

:::figure{src="images/fig_5_70.jpg" id="fig_5_70"}
:::

:::solution{label="हल"}
हल हेक्साऐमीनक्रोमियम (III) आयन $\left[\mathrm{Cr}\left(\mathrm{NH}_{3}\right)_{6}\right]^{3+}$ आयन
Cr की ऑक्सीकरण अवस्था $=+3$

$$
\begin{aligned}
& 24 \mathrm{Cr}=[\mathrm{Ar}] 3 d^{5} 4 s^{1} \\
& \mathrm{Cr}^{3+}=[\mathrm{Ar}] 3 d^{3}
\end{aligned}
$$

$\mathrm{Cr}^{3+}$ छह $\mathrm{NH}_{3}$ अणुओं से छः युग्मित इलेक्ट्रॉनों के लिए छ: रिक्त कोश उपलब्ध कराता है। यह $d^{2} s p^{3}$ संकरित तथा अष्टफलकीय है।

यह तीन अयुग्मित इलेक्ट्रॉनों की उपस्थिति के कारण अनुचुम्बकीय है।


टेट्रासायनोनिकलेट (II) आयन $\left[\mathrm{Ni}\left(\mathrm{CN}_{4}\right)^{2-}\right.$

$$
\begin{aligned}
\mathrm{Ni} & =[\mathrm{Ar}] 3 d^{8} 4 s^{2} \\
\mathrm{Ni}^{2+} & =[\mathrm{Ar}] 3 d^{8}
\end{aligned}
$$

$$
\mathrm{Ni}^{2+}=\begin{array}{|l|l|l|l|l|}
\hline 1 \mathrm{l} & 1 \mathrm{l} & 1 \mathrm{l} & 1 & 1 \\
\hline
\end{array} \quad \begin{array}{|l|l|}
4 \mathrm{~s} \\
\hline
\end{array} \quad \begin{array}{|l|}
\hline \\
\hline
\end{array}
$$

$\mathrm{CN}^{-}$प्रबल क्षेत्र लिगेण्ड होने के कारण $3 d$-कक्षकों में इलेक्ट्रॉनों को युग्मित कर देता है। जिसके कारण इन $3 d$-कक्षकों में से एक कक्षक, $\mathrm{CN}^{-}$आयन के लिए रिक्त हो जाता है।

$$
\left[\mathrm{Ni}(\mathrm{CN})_{4}\right]^{2-}=\underbrace{\begin{array}{|l|l|l|l|l|}
\hline 1 l & 1 l & 1 l & 1 l & \times \infty \\
\hline
\end{array}}_{\substack{\text { सभी इलेक्ट्रॉन } \\
\text { युग्मित हैं }}}
$$

इसमें अयुग्मित इलेक्ट्रॉन उपस्थित नहीं है अत: यह प्रतिचुम्बकीय है।
:::

:::

:::question{number="5.20" kind="exercise" id="q_5.20" topic="Colour difference in nickel complexes" corrections_applied="1"}
#### प्रश्न 5.20

:::prompt
$\left[\mathrm{Ni}\left(\mathrm{H}_{2} \mathrm{O}\right)_{6}\right]^{2+}$ का विलयन हरा है परंतु $\left[\mathrm{Ni}(\mathrm{CN})_{4}\right]^{2-}$ का विलयन रंगहीन है। समझाइए।
:::

:::solution{label="हल"}
हल $\mathrm{Ni}^{2+}$ आयन का विन्यास

$$
\mathrm{Ni}^{2+}=[\mathrm{Ar}] 3 d^{8}
$$

$$
\mathrm{Ni}^{2+}=\begin{array}{|l|l|l|l|l|}
\hline ് ഡ & \frac{3 d^{8}}{1 l} & 1 l & 1 & 1 \\
\hline
\end{array} \quad \begin{array}{|l|}
\hline \\
\hline
\end{array} \quad \begin{array}{|l|l|}
\hline 4 s & \\
\hline
\end{array}
$$

$\left[\mathrm{Ni}\left(\mathrm{H}_{2} \mathrm{O}\right)_{6}\right]^{2+}$ संकुल आयन में $\mathrm{H}_{2} \mathrm{O}$ अणु दुर्बल क्षेत्र लिगेण्ड हैं अत: ये इलेक्ट्रॉनों का युग्मन नहीं करते है। परिणामस्वरूप संकुल के पास दो अयुग्मित इलेक्ट्रॉन उपस्थित रहते हैं। इस प्रकार $d-d$ संक्रमण के कारण यदि संकुल लाल क्षेत्र की ऊर्जा के संगत प्रकाश का अवशोषण करता है तो हरे क्षेत्र की ऊर्जा के संगत प्रकाश का उत्सर्जन होता है। अतः यह हरा है। $\left[\mathrm{Ni}(\mathrm{CN})_{4}\right]^{2-}$ संकुल आयन में, $\mathrm{CN}^{-}$आयन प्रबल क्षेत्र लिगेण्ड है। अतः $\mathrm{CN}^{-}$आयनों की उपस्थिति में 3d-कक्षकों में उपस्थित दो अयुग्मित इलेक्ट्रॉन, युग्मित हो जाते हैं। इस प्रकार कोई अयुग्मित इलेक्ट्रॉन न होने के कारण $d-d$ संक्रमण नहीं होता है। अतः यह रंगहीन है।
:::

:::

:::question{number="5.21" kind="exercise" id="q_5.21" topic="Colour difference in iron complexes"}
#### प्रश्न 5.21

:::prompt
$\left[\mathrm{Fe}(\mathrm{CN})_{6}\right]^{4-}$ तथा $\left[\mathrm{Fe}\left(\mathrm{H}_{2} \mathrm{O}\right)_{6}\right]^{2+}$ के तनु विलयनों के रंग भिन्न होते हैं। क्यों?
:::

:::solution{label="हल"}
हल दोनों संकुल यौगिकों में Fe की ऑक्सीकरण अवस्था $+2$ तथा विन्यास $3 d^{6}$ है। अर्थात् इनके पास चार अयुग्मित इलेक्ट्रॉन है। दुर्बल क्षेत्र लिगेण्ड, $\mathrm{H}_{2} \mathrm{O}$ की उपस्थिति में अयुग्मित इलेक्ट्रॉनों का युग्मन नहीं हो पाता है जबकि प्रबल क्षेत्र लिगेण्ड, $\mathrm{CN}^{-}$की उपस्थिति में इलेक्ट्रॉनों का युग्मन हो जाता है। अतः कोई अयुग्मित इलेक्ट्रॉन नहीं रहता है। अयुग्मित इलेक्ट्रॉनों की संख्या में अंतर के कारण दोनों संकुल आयन भिन्न रंग देते हैं।
:::

:::

:::question{number="5.22" kind="exercise" id="q_5.22" topic="Bonding in metal carbonyls" corrections_applied="2"}
#### प्रश्न 5.22

:::prompt
धातु कार्बोनिलों में आबंध की प्रकृति की विवेचना कीजिए।
:::

:::figure{src="images/fig_5_71.jpg" id="fig_5_71"}
कार्बोनिल संकुल में सहक्रियाशीलता आबंघन अन्योन्य क्रिया का उदाहरण
:::

:::solution{label="हल"}
हल धातु कार्बोनिलों के धातु-कार्बन आबंध में $s$-तथा $p$-दोनों के गुण पाए जाते हैं। $M-C\sigma$ आबंध कार्बोनिल समूह के कार्बन पर उपस्थित इलेक्ट्रॉन युग्म को धातु के रिक्त कक्षक में दान करने से बनता है। $M-C \pi$ आबंध धातु के पूरित $d$-कक्षकों में से एक इलेक्ट्रॉन युग्म को कार्बन

मोनोऑक्साइड के रिक्त प्रतिआबंधन $\pi$ कक्षक में दान करने से बनता है। धातु से लिगेण्ड का आबंध एक सहक्रियाशीलता का प्रभाव उत्पन्न करता है जो CO व धातु के मध्य आबंध को मजबूत बनाता है।
:::

:::

:::question{number="5.23" kind="exercise" id="q_5.23" topic="Oxidation state and coordination number" corrections_applied="5"}
#### प्रश्न 5.23

:::prompt
निम्न संकुलों में केंद्रीय धातु आयन की ऑक्सीकरण अवस्था, $d$ कक्षकों का अधिग्रहण एवं उपसहसंयोजन संख्या बतलाइए-
:::

:::part{label="(i)"}
:::prompt
$\mathrm{K}_{3}\left[\mathrm{Co}\left(\mathrm{C}_{2} \mathrm{O}_{4}\right)_{3}\right]$
:::

:::

:::part{label="(ii)"}
:::prompt
cis- $\left[\mathrm{CrCl}_{2}(\text { en })_{2}\right] \mathrm{Cl}$
:::

:::

:::part{label="(iii)"}
:::prompt
$\left(\mathrm{NH}_{4}\right)_{2}\left[\mathrm{CoF}_{4}\right]$
:::

:::

:::part{label="(iv)"}
:::prompt
$\left[\mathrm{Mn}\left(\mathrm{H}_{2} \mathrm{O}\right)_{6}\right] \mathrm{SO}_{4}$
:::

:::

:::solution{label="हल"}
हल (i) $\mathrm{K}_{3}\left[\mathrm{Co}\left(\mathrm{C}_{2} \mathrm{O}_{4}\right)_{3}\right]$
माना Co की ऑक्सीकरण अवस्था $x$ है।

$$
\begin{aligned}
(+1) 3+x+(-2) \times 3 & =0 \\
+3+x-6 & =0 \\
x & =+3
\end{aligned}
$$

अत: Co की ऑक्सीकरण अवस्था $+3$ है।

$$
\begin{aligned}
& 2{ }^{27} \mathrm{Co}=[\mathrm{Ar}] 3 d^{7} 4 s^{2} \\
& \mathrm{Co}^{3+}=[\mathrm{Ar}] 3 d^{6}
\end{aligned}
$$

अतः $d$-कक्षक का विन्यास $d^{6}$ या $t_{2 g}^{6} e_{g}^{0}$ है।
( $\because \mathrm{C}_{2} \mathrm{O}_{4}^{2-}$ प्रबल क्षेत्र लिगेण्ड है।)
Co की उपसहसंयोजन संख्या $=3 \times \mathrm{C}_{2} \mathrm{O}_{4}$ की दंतुरता

$$
=3 \times 2 \quad\left(\because \mathrm{C}_{2} \mathrm{O}_{4}^{2-}\right. \text { एक द्विदंतुर लिगेण्ड है।) }
$$

(ii) $\mathbf{c i s}-\left[\mathbf{C r}(\mathbf{e n})_{\mathbf{2}} \mathbf{C l}_{\mathbf{2}}\right] \mathbf{C l}$
माना Cr की ऑक्सीकरण अवस्था $x$ है।
$$
\begin{aligned}
x+(0) \times 2+(-1) \times 2+(-1) & =0 \\
x+0-2-1 & =0 \\
x & =+3
\end{aligned}
$$
अत: Cr की ऑक्सीकरण अवस्था $+3$ है।
$$
\begin{aligned}
\mathrm{Cr} & =[\mathrm{Ar}] 3 d^{5} 4 s^{1} \\
\mathrm{Cr}^{3+} & =[\mathrm{Ar}] 3 d^{3}
\end{aligned}
$$
अतः $d$-कसकों का विन्यास $d^{3}$ या $t_{2 g}^{3}, e_{g}^{0}$ है।
Cr की उपसहसंयोजन संख्या $=2 \times$ en की दंतुरता $+2=2 \times 2+2=6$
(iii) $\left(\mathrm{NH}_{4}\right)_{2}\left[\mathrm{CoF}_{4}\right]$
माना Co की ऑक्सीकरण अवस्था $x$ है।
$$
\begin{aligned}
(+1) \times 2+x+(-1) \times 4 & =0 \\
2+x-4 & =0 \\
x & =+2
\end{aligned}
$$
अत: Co की ऑक्सीकरण अवस्था $+2$ है।
$$
\begin{aligned}
& 27 \mathrm{Co}=[\mathrm{Ar}] 3 d^{7} 4 s^{2} \\
& \mathrm{Co}^{2+}=[\mathrm{Ar}] 3 d^{7}
\end{aligned}
$$
अतः $d$-कक्षकों का विन्यास $d^{7}$ या $t_{2 g}^{5} e_{g}^{2}$ है।
( ∵ F ${ }^{-}$एक दुर्बल क्षेत्र लिगेण्ड है।)
Co की उपसहसंयोजन संख्या $=4$
(iv) $\left[\mathrm{Mn}\left(\mathrm{H}_{2} \mathrm{O}\right)_{6}\right] \mathrm{SO}_{4}$
माना Mn की ऑक्सीकरण अवस्था $x$ है।
$$
\begin{aligned}
x+(0) \times 6+(-2) & =0 \\
x & =+2
\end{aligned}
$$
अत: Mn की ऑक्सीकरण अवस्था $+2$ है।
$$
\begin{aligned}
\mathrm{Mn} & =[\mathrm{Ar}] 3 d^{5} 4 s^{2} \\
\mathrm{Mn}^{2+} & =[\mathrm{Ar}] 3 d^{5}
\end{aligned}
$$
अतः $d$-कसकों का विन्यास $d^{5}$ या $t_{2 g}^{3} e_{g}^{2}$ है।
Mn की उपसहसंयोजन संख्या $=6$
:::

:::

:::question{number="5.24" kind="exercise" id="q_5.24" topic="IUPAC names and complex properties" corrections_applied="6"}
#### प्रश्न 5.24

:::prompt
निम्न संकुलों के IUPAC नाम लिखिए तथा ऑक्सीकरण अवस्था, इलेक्ट्रॉनिक विन्यास और उपसहसंयोजन संख्या दर्शाइए। संकुल का त्रिविम रसायन तथा चुंबकीय आघूर्ण भी बतलाइए:
:::

:::figure{src="images/fig_5_72.jpg" id="fig_5_72"}
समपक्ष रूप
:::

:::figure{src="images/fig_5_73.jpg" id="fig_5_73"}
विपक्ष रूप
:::

:::figure{src="images/fig_5_99.svg" id="fig_5_99"}
:::

:::figure{src="images/fig_5_74.jpg" id="fig_5_74"}
:::

:::figure{src="images/fig_5_100.svg" id="fig_5_100"}
दर्पण
:::

:::figure{src="images/fig_5_75.jpg" id="fig_5_75"}
:::

:::figure{src="images/fig_5_101.svg" id="fig_5_101"}
fac-nq
(फलकीय रूप)
:::

:::figure{src="images/fig_5_102.svg" id="fig_5_102"}
mer-रूप
(रेखाशिक रूप)
:::

:::part{label="(i)"}
:::prompt
$\mathrm{K}\left[\mathrm{Cr}\left(\mathrm{H}_{2} \mathrm{O}\right)_{2}\left(\mathrm{C}_{2} \mathrm{O}_{4}\right)_{2}\right] \cdot 3 \mathrm{H}_{2} \mathrm{O}$
:::

:::

:::part{label="(ii)"}
:::prompt
$\left[\mathrm{CrCl}_{3}(\mathrm{py})_{3}\right]$
:::

:::

:::part{label="(iii)"}
:::prompt
$\left[\mathrm{Co}\left(\mathrm{NH}_{3}\right)_{5} \mathrm{Cl}_{-}\right] \mathrm{Cl}_{2}$
:::

:::

:::part{label="(iv)"}
:::prompt
$\mathrm{Cs}\left[\mathrm{FeCl}_{4}\right]$
:::

:::

:::part{label="(v)"}
:::prompt
$\mathrm{K}_{4}\left[\mathrm{Mn}(\mathrm{CN})_{6}\right]$
:::

:::

:::solution{label="हल"}
हल (i) $\mathrm{K}\left[\mathrm{Cr}\left(\mathrm{H}_{2} \mathrm{O}\right)_{2}\left(\mathrm{C}_{2} \mathrm{O}_{4}\right)_{2}\right] \cdot 3 \mathrm{H}_{2} \mathrm{O}$
IUPAC नाम पोटैशियमडाइएक्वाडाइऑक्सेलेटोक्रोमेट (III) हाइड्रेट
उपसहसंयोजन संख्या $(\mathrm{CN})=6$
आकार = अष्टफलकीय
Cr की ऑक्सीकरण अवस्था

$$
\begin{aligned}
+1+x+(0) \times 2+(-2) \times 2+3(0) & =0 \\
+1+x-4 & =0 \\
x & =+3
\end{aligned}
$$

$\mathrm{Cr}^{3+}$ का इलेक्ट्रॉनिक विन्यास $=3 d^{3}=t_{2 g}^{3} \cdot e_{g}^{0}$
अयुग्मित इलेक्ट्रॉन $(n)=3$
चुम्बकीय आघूर्ण $(\mu)=\sqrt{n(n+2)}=\sqrt{3(3+2)}=3.87 \mathrm{BM}$
यह संकुल ज्यामितीय समावयवता दर्शाता है। इसके समपक्ष और विपक्ष रूप निम्न हैं।


समपक्ष रूप $d$-तथा $l$-रूप भी दर्शाता है (अर्थात् ध्रुवण समावयवता)


(ii) $\mathrm{CrCl}_{3}(\mathrm{py})_{3}$
IUPAC नाम ट्राइक्लोरोडो पिरीडीन क्रोमियम (III)
Cr की ऑक्सीकरण अवस्था
$$
\begin{aligned}
x+(-1) \times 3+(0) \times 3 & =0 \\
x & =+3
\end{aligned}
$$
उपसहसंयोजन संख्या $(\mathrm{CN})=6$
$\mathrm{Cr}^{3+}$ का बाह्य इलेक्ट्रॉनिक विन्यास $=3 d^{3}\left(t_{2 g}^{3} e_{g}^{0}\right)$
अयुग्मित इलेक्ट्रॉनों की संख्या $=3$
अत: चुम्बकीय आघूर्ण $\mu=\sqrt{n(n+2)}$ BM
$$
\begin{aligned}
& =\sqrt{3(3+2)} \\
& =\sqrt{15} \\
& =3.87 \mathrm{BM}
\end{aligned}
$$
यह संकुल फलकीय (fac) तथा रेखांशिक (mer)-ज्यामितीय समावयवता दर्शाता है (फलकीय रूप में एक जैसे समूह पास-पास होते हैं जबकि रेखांशिक रूप में यह सत्य नहीं है)।

(iii) $\left[\mathrm{Co}\left(\mathrm{NH}_{3}\right)_{5} \mathrm{Cl}\right] \mathrm{Cl}_{2}$
IUPAC नाम पेन्टाऐमीनक्लोरोकोबाल्ट (III) क्लोराइड
Co की ऑक्सीकरण अवस्था की गणना इस प्रकार कर सकते हैं।

$$
\begin{aligned}
x+0 \times 5+(-1)+(-1) \times 2 & =0 \\
x-3 & =0 \\
x & =+3
\end{aligned}
$$

उपसहसंयोजन संख्या $(\mathrm{CN})=5+1=6$ (एकदंतुर लिगेण्डों की संख्या)
$\mathrm{Co}^{3+}$ का बाह्य इलेक्ट्रॉनिक विन्यास $=3 d^{6}=t_{2 g}^{6} e_{g}^{0}$
अतः चुम्बकीय आघूर्ण, $\mu=\sqrt{n(n+2)} B M$

$$
=\sqrt{0(0+2)} \mathrm{BM}=0 \mathrm{BM}
$$

यह संकुल ज्यामितीय के साथ-साथ ध्रुवण समावयवता भी नहीं दर्शाता है।


(iv) $\mathbf{C s}\left[\mathrm{FeCl}_{4}\right]$
IUPAC नाम सीजियम टेट्राक्लोरोफैरेट (III)
Fe की ऑक्सीकरण अवस्था

$$
\begin{aligned}
+1+x+(-1) \times 4 & =0 \\
x-3 & =0 \\
x & =+3
\end{aligned}
$$

उपसहसंयोजन संख्या, $(\mathrm{CN})=4$
Fe का बाह्य इलेक्ट्रॉनिक विन्यास $=3 d^{5}\left(t_{2 g}^{3} e_{g}^{2}\right)$
अयुग्मित इलेक्ट्रॉनों की संख्या $=5$

∴ चुम्बकीय आघूर्ण $\mu=\sqrt{n(n+2)}$ BM
$$
=\sqrt{5(5+2)}=\sqrt{35}=5.92 \mathrm{BM}
$$
यह चतुष्फलकीय संकुल है अतः यह ज्यामितीय या ध्रुवण समावयवता नहीं दर्शाता है।
(v) $\mathrm{K}_{4}\left[\mathrm{Mn}(\mathrm{CN})_{6}\right]$
IUPAC नाम पोटैशियम हेक्सासायनोमैंग्नेट (II)
$\mathrm{K}_{4}\left[\mathrm{Mn}(\mathrm{CN})_{6}\right]$ में Mn की ऑक्सीकरण अवस्था
$$
\begin{aligned}
(+1) \times 4+x+(-1) \times 6 & =0 \\
x-2 & =0 \\
x & =+2
\end{aligned}
$$
उपसहसंयोजन संख्या $(\mathrm{CN})=6$
Mn का बाह्य इलेक्ट्रॉनिक विन्यास $=3 d^{5}\left[t_{2 g}^{5} e_{g}^{0}\right]$
( $\because \mathrm{CN}^{-}$प्रबल क्षेत्र लिगेण्ड है।)
अयुग्मित इलेक्ट्रॉनों की संख्या $=1$
चुम्बकीय आघूर्ण $\mu=\sqrt{n(n+2)} B M=\sqrt{1(1+2)} B M$
$$
=\sqrt{3}=1.73 \mathrm{BM} .
$$
यह संकुल त्रिविम समावयवता नहीं दर्शाता है।
:::

:::

:::question{number="5.25" kind="exercise" id="q_5.25" topic="Colour of titanium complex"}
#### प्रश्न 5.25

:::prompt
क्रिस्टल क्षेत्र सिद्धांत के आधार पर संकुल $\left[\mathrm{Ti}\left(\mathrm{H}_{2} \mathrm{O}\right)_{6}\right]^{3+}$ के बैंगनी रंग की व्याख्या कीजिए।
:::

:::

:::question{number="5.26" kind="exercise" id="q_5.26" topic="Chelate effect" corrections_applied="3"}
#### प्रश्न 5.26

:::prompt
कीलेट प्रभाव से क्या तात्पर्य है? एक उदाहरण दीजिए।
:::

:::figure{src="images/fig_5_103.svg" id="fig_5_103"}
:::

:::solution{label="हल"}
हल जब एक द्विदंतुर अथवा बहुदंतुर लिगेण्ड अपने दो या अधिक दाता परमाणुओं का प्रयोग एक ही धातु आयन से आबंधन के लिए इस प्रकार करता है कि $5$ तथा $6$ सदस्यों वाली शृंखला बनती हो तो यह प्रभाव कीलेट प्रभाव कहलाता है।

उदाहरण


या $\left[\mathrm{PtCl}_{2}(\right.$ en $\left.)\right]$
:::

:::

:::question{number="5.27" kind="exercise" id="q_5.27" topic="Applications of coordination compounds" corrections_applied="16"}
#### प्रश्न 5.27

:::prompt
प्रत्येक का एक उदाहरण देते हुए निम्नलिखित में उपसहसंयोजन यौगिकों की भूमिका की संक्षिप्त विवेचना कीजिए-
:::

:::part{label="(i)"}
:::prompt
जैव प्रणालियाँ
:::

:::

:::part{label="(ii)"}
:::prompt
औषध रसायन
:::

:::

:::part{label="(iii)"}
:::prompt
विश्लेषणात्मक रसायन
:::

:::

:::part{label="(iv)"}
:::prompt
धातुओं का निष्कर्षण/धातु कर्म।
:::

:::

:::solution{label="हल"}
हल (i) जैव प्रणालियों में उपसहसंयोजन यौगिकों की भूमिका

(a) रक्त का लाल वर्णक हीमोग्लोबिन, जोकि ऑक्सीजन का वाहक है, पोरफाइरिन के साथ $\mathrm{Fe}^{2+}$ का संकुल है।
(b) प्रकाश संश्लेषण के लिए उत्तरदायी वर्णक, क्लोरोफिल, मैग्नीशियम $\left(\mathrm{Mg}^{2+}\right)$ का पोरफाइरिन के साथ संकुल है।
(c) विटामिन $\mathrm{B}_{12}$ सायनाकोबाल एमीन, (प्रति प्रणाली अरक्तता कारक) कोबाल्ट का एक उपसहसंयोजक यौगिक है।

(ii) औषध रसायन में उपसहसंयोजक यौगिकों की भूमिका
    (a) प्लेटिनम के संकुल समपक्ष $\left[\mathrm{Pt}\left(\mathrm{NH}_{3}\right)_{2} \mathrm{Cl}_{2}\right]$ (समपक्ष प्लेटिन), का उपयोग ट्यूमर वृद्धि को प्रभावी रूप से रोकने में किया जाता है।
    (b) कैल्सियम के संकुल EDTA को लेड की विषाक्तता के उपचार में प्रयुक्त किया जाता है। Ca-EDTA एक दुर्बल संकुल है। शरीर में संकुल का कैल्सियम, शरीर में उपस्थित लैड से प्रतिस्थापित हो जाता है तथा मूत्र के साथ बाहर आ जाता है।
    (c) जीव-जन्तु निकायों में कॉपर तथा आयरन की अधिकता को D-पेनिसिलऐमीन तथा डेसफेरीऑक्सिम B लिगेण्डों के साथ उपसहसंयोजन यौगिक बनाकर दूर किया जाता है।
(iii) विश्लेषणात्मक रसायन में उपसहसंयोजक यौगिकों की भूमिका गुणात्मक तथा मात्रात्मक रासायनिक विश्लेषणों में उपसहसंयोजन यौगिकों के अनेक उपयोग है।

(a) गुणात्मक रासायनिक विश्लेषण
$\mathrm{Cu}^{2+}$ आयन की जाँच नीले रंग के संकुल टेट्राऐमीनकॉपर (II) आयन के बनने पर निर्भर है।

$$
\mathrm{Cu}^{2+}+4 \mathrm{NH}_{3} \longrightarrow \underset{\text { गहरा नीला }}{\left[\mathrm{Cu}\left(\mathrm{NH}_{3}\right)_{4}\right]^{2+}}
$$

$\mathrm{Ni}^{2+}$ आयन की जाँच लाल रंग के संकुल डाइमेथिल ग्लाइऑक्सिम के बनने से की जाती है।

$$
\mathrm{NiCl}_{2}+2 \mathrm{DMG}+2 \mathrm{NH}_{4} \mathrm{OH} \longrightarrow \underset{\substack{\text { निकल बिस डाइमेथिल } \\ \text { ग्लाइऑक्सिमेट }}}{\left[\mathrm{Ni}(\mathrm{DMG})_{2}\right]}+2 \mathrm{NH}_{4} \mathrm{Cl}+2 \mathrm{H}_{2} \mathrm{O}
$$

समूह I में $\mathrm{Ag}^{+}$तथा $\mathrm{Hg}^{2+}$ में अन्तर इस तथ्य पर आधारित है कि AgCl , अमोनिया में घुलकर घुलनशील संकुल बनाता है जबकि $\mathrm{Hg}_{2} \mathrm{Cl}_{2}$, अघुलनशील (अविलेय) पदार्थ बनाता है।

$$
\begin{aligned}
\mathrm{AgCl}(s)+2 \mathrm{NH}_{3}(\mathrm{aq}) & \longrightarrow \underset{\text { विलेय संकुल }}{\left[\mathrm{Ag}\left(\mathrm{NH}_{3}\right)_{2}\right] \mathrm{Cl}(\mathrm{aq})} \\
. \mathrm{Hg}_{2} \mathrm{Cl}_{2}(s)+\mathrm{NH}_{3} & \longrightarrow \underset{\text { काला अविलेय पदार्थ }}{\mathrm{Hg}\left(\mathrm{NH}_{2}\right) \mathrm{Cl} \cdot \mathrm{Hg}}+\mathrm{Cl}^{-}
\end{aligned}
$$

(b) मात्रात्मक रासायनिक विश्लेषण में
$\mathrm{Ni}^{2+}$ का आकलन, $\mathrm{Ni}^{2+}$ को अमोनिया की उपस्थिति में निकैल डाइमेथिल ग्लाइऑक्सिम संकुल का लाल अवक्षेप प्राप्त करके की जाती है।
$$
\mathrm{Ni}^{2+}+2 \mathrm{DMG} \xrightarrow{\mathrm{NH}_{3}} \underset{\text { लाल अवक्षेप }}{\left[\mathrm{Ni}(\mathrm{DMG})_{2}\right]}
$$
$\mathrm{Ca}^{2+}, \mathrm{Zn}^{2+}, \mathrm{Fe}^{2+}, \mathrm{Co}^{2+}, \mathrm{Ni}^{2+}$ आदि आयनों के आकलन में EDTA का उपयोग किया जाता है।
(iv) धातुओं के निष्कर्षण/धातुकर्म में उपसहसंयोजन यौगिकों की भूमिका
विभिन्न धातुओं की कुछ प्रमुख निष्कर्षण विधियों में जैसे सिल्वर तथा गोल्ड के लिए संकुल विरचन का उपयोग होता है।

$$
4 \mathrm{Au}+8 \mathrm{KCN}+2 \mathrm{H}_{2} \mathrm{O}+\mathrm{O}_{2} \longrightarrow 4 \mathrm{~K}\left[\mathrm{Au}(\mathrm{CN})_{2}\right]+4 \mathrm{KOH}
$$

पोटैशियम डाइसायनो ऑरेट

$$
2 \mathrm{~K}\left[\mathrm{Au}(\mathrm{CN})_{2}\right]+\mathrm{Zn} \longrightarrow \mathrm{~K}_{2}\left[\mathrm{Zn}(\mathrm{CN})_{4}\right]+2 \mathrm{Au} \downarrow
$$

कुछ धातुओं के शुद्धिकरण में भी संकुल विरचन का उपयोग होता है।
उदाहरण मॉन्ड प्रक्रम में अशुद्ध निकेल को संकुल $\left[\mathrm{Ni}(\mathrm{CO})_{4}\right]$ में परिवर्तित करके तथा
इसका अपघटन करके शुद्ध निकेल प्राप्त की जाती है।
:::

:::

:::question{number="5.28" kind="exercise" id="q_5.28" topic="Ions produced in solution"}
#### प्रश्न 5.28

:::prompt
संकुल $\left[\mathrm{Co}\left(\mathrm{NH}_{3}\right)_{6}\right] \mathrm{Cl}_{2}$ से विलयन में कितने आयन उत्पन्न होंगे-
(i) $6$
(ii) $4$
(iii) $3$
(iv) $2$
:::

:::solution{label="हल"}
हल संकुल $\left[\mathrm{Co}\left(\mathrm{NH}_{3}\right)_{6}\right] \mathrm{Cl}_{2}$, जलीय विलयन में वियोजित होकर $3$ आयन देगा।

$$
\mathrm{Co}\left(\mathrm{NH}_{3}\right)_{6} \mathrm{Cl}_{2} \longrightarrow\left[\mathrm{Co}\left(\mathrm{NH}_{3}\right)_{6}\right]^{2+}+2 \mathrm{Cl}^{-}
$$
:::

:::answer
**उत्तर:** अतः सही उत्तर (c) है।
:::

:::

:::question{number="5.29" kind="exercise" id="q_5.29" topic="Highest magnetic moment ion"}
#### प्रश्न 5.29

:::prompt
निम्नलिखित आयनों में से किसके चुंबकीय आघूर्ण का मान सर्वाधिक होगा?
(i) $\left[\mathrm{Cr}\left(\mathrm{H}_{2} \mathrm{O}\right)_{6}\right]^{3+}$
(ii) $\left[\mathrm{Fe}\left(\mathrm{H}_{2} \mathrm{O}\right)_{6}\right]^{2+}$
(iii) $\left[\mathrm{Zn}\left(\mathrm{H}_{2} \mathrm{O}\right)_{6}\right]^{2+}$
:::

:::solution{label="हल"}
हल दिए गए संकुलों में धातुओं की ऑक्सीकरण अवस्थाएँ इनके इलेक्ट्रॉनिक विन्यास के साथ इस प्रकार है।
(i) $\mathrm{Cr}^{3+}: 3 d^{3}$ विन्यास, अयुग्मित इलेक्ट्रॉनों की संख्या $(n)=3$
(ii) $\mathrm{Fe}^{2+}: 3 d^{6}$ विन्यास, $n=4$
(iii) $\mathrm{Zn}^{2+}: 3 d^{10}$ विन्यास, $n=0$

संकुल $\left[\mathrm{Fe}\left(\mathrm{H}_{2} \mathrm{O}\right)_{6}\right]^{2+}$ में अधिकतम अयुग्मित इलेक्ट्रॉन है अत: इसके चुम्बकीय आघूर्ण का मान सर्वाधिक होगा।
:::

:::answer
**उत्तर:** अतः सही उत्तर (b) है।
:::

:::

:::question{number="5.30" kind="exercise" id="q_5.30" topic="Oxidation number of cobalt"}
#### प्रश्न 5.30

:::prompt
K[Co(CO) ${ }_{4}$ ] में कोबाल्ट की ऑक्सीकरण संख्या है-
(i) +1
(ii) +3
(iii) -1
(iv) -3
:::

:::solution{label="हल"}
हल माना Co की ऑक्सीकरण संख्या $=x$

$$
\begin{aligned}
+.1+x+4 \times 0 & =0 \\
x & =-1
\end{aligned}
$$
:::

:::answer
**उत्तर:** अतः सही उत्तर (c) है।
:::

:::

:::question{number="5.31" kind="exercise" id="q_5.31" topic="Most stable complex" corrections_applied="1"}
#### प्रश्न 5.31

:::prompt
निम्न में सर्वाधिक स्थायी संकुल है-
(i) $\left[\mathrm{Fe}\left(\mathrm{H}_{2} \mathrm{O}\right)_{6}\right]^{3+}$
(ii) $\left[\mathrm{Fe}\left(\mathrm{NH}_{3}\right)_{6}\right]^{3+}$
(iii) $\left[\mathrm{Fe}\left(\mathrm{C}_{2} \mathrm{O}_{4}\right)_{3}\right]^{3-}$
(iv) $\left[\mathrm{FeCl}_{6}\right]^{3-}$
:::

:::solution{label="हल"}
हल इन सभी संकुलों में Fe की ऑक्सीकरण अवस्था $+3$ है। हालाँकि संकुल (c) एक कीलेट है क्योंकि तीन ऑक्सेलेट आयन कीलेट लिगेण्ड का कार्य करते हैं। अतः $\left[\mathrm{Fe}\left(\mathrm{C}_{2} \mathrm{O}_{4}\right)_{3}\right]^{3-}$ सर्वाधिक स्थायी संकुल है।
:::

:::answer
**उत्तर:** इस प्रकार सही उत्तर (c) है।
:::

:::

:::question{number="5.32" kind="exercise" id="q_5.32" topic="Order of absorption wavelength" corrections_applied="1"}
#### प्रश्न 5.32

:::prompt
निम्नलिखित के लिए दृश्य प्रकाश में अवशोषण की तरंगदैर्ध्य का सही क्रम क्या होगा?
$\left[\mathrm{Ni}\left(\mathrm{NO}_{2}\right)_{6}\right]^{4-},\left[\mathrm{Ni}\left(\mathrm{NH}_{3}\right)_{6}\right]^{2+},\left[\mathrm{Ni}\left(\mathrm{H}_{2} \mathrm{O}\right)_{6}\right]^{2+}$
:::

:::solution{label="हल"}
हल सभी दिए गए संकुलों में Ni धातु, $+2$ ऑक्सीकरण अवस्था में है। स्पेक्ट्रमी रासायनिक श्रेणी के अनुसार, लिगेण्डों की प्रबलता का क्रम निम्नानुसार है।

$$
\mathrm{H}_{2} \mathrm{O}<\mathrm{NH}_{3}<\mathrm{NO}_{2}^{-}
$$

उत्तेजन के लिए ऊर्जा के अवशोषण का क्रम निम्न प्रकार होगा

$$
\left[\mathrm{Ni}\left(\mathrm{H}_{2} \mathrm{O}\right)_{6}\right]^{2+}<\left[\mathrm{Ni}\left(\mathrm{NH}_{3}\right)_{6}\right]^{2+}<\left[\mathrm{Ni}\left(\mathrm{NO}_{2}\right)_{6}\right]^{4-} \quad \because E=\frac{h \mathrm{c}}{\lambda} \text { या } E \propto \frac{1}{\lambda} ;
$$

अतः तरंगदैध्यों का क्रम विपरीत होगा, यह इस प्रकार है।
:::

:::answer
**उत्तर:** $$
\left[\mathrm{Ni}\left(\mathrm{H}_{2} \mathrm{O}\right)_{6}\right]^{2+}>\left[\mathrm{Ni}\left(\mathrm{NH}_{3}\right)_{6}\right]^{2+}>\left[\mathrm{Ni}\left(\mathrm{NO}_{2}\right)_{6}\right]^{4-}
$$
:::

:::

## अतिरिक्त प्रश्न

:::question{number="5.1" kind="additional_exercise" id="it_5.1" topic="Formulas of coordination compounds" corrections_applied="2"}
#### अतिरिक्त प्रश्न 5.1

:::prompt
निम्नलिखित उपसहसंयोजन यौगिकों के सूत्र लिखिए-

(i) टेट्राऐम्मीनडाइएक्वाकोबाल्ट (III) क्लोराइड
(ii) पोटैशियम टेट्रासायनिडोनिकैलेट (II)
(iii) ट्रिस(एथेन-1, 2-डाइऐमीन)क्रोमियम (III) क्लोराइड
(iv) ऐम्मीनब्रोमिडोक्लोरिडोनाइट्रिटो-N-प्लैटिनेट (II)
(v) डाइक्लोरोबिस(एथेन-1, 2-डाइऐमीन) प्लैटिनम (IV) नाइट्रेट
(vi) आयरन (III) हेक्सासायनिडोफेरेट(II)
:::

:::solution{label="हल"}
(i) टेट्राऐमीनडाइएक्वा कोबाल्ट (III) क्लोराइड
लिगेण्ड धातु प्रतिआयन
III
$\left[\mathrm{Co}\left(\mathrm{NH}_{3}\right)_{4}\left(\mathrm{H}_{2} \mathrm{O}\right)_{2}\right] \mathrm{Cl}_{\mathrm{x}}$
$x$ का मान ज्ञात करने के लिए हम संकुल पर आवेश ज्ञात करेंगे।

$$
\begin{aligned}
{\left[\mathrm{Co}\left(\mathrm{NH}_{3}\right)_{4}\left(\mathrm{H}_{2} \mathrm{O}\right)_{2}\right]^{x+} } & \\
+3+4 \times 0+2 \times 0 & =x \\
x & =+3
\end{aligned}
$$

अतः संकुल का सूत्र $\left[\mathrm{Co}\left(\mathrm{NH}_{3}\right)_{4}\left(\mathrm{H}_{2} \mathrm{O}\right)_{2}\right] \mathrm{Cl}_{3}$ है।

(ii) पोटैशियम टेट्रासायनो निकेलेट (II)
प्रतिआयन लिगेण्ड धातु
$$
\mathrm{K}_{x}\left[\mathrm{Ni}(\mathrm{CN})_{4}\right]
$$
$x$ का मान ज्ञात करने के लिए पहले संकुल पर आवेश ज्ञात करेंगे।

$\left[\mathrm{Ni}(\mathrm{CN})_{4}\right]^{\mathrm{x}-}$ (जैसा कि $\mathrm{K}^{+}$घनायन है)
या

$$
\begin{aligned}
+2+(-1) \times 4 & =-x \\
-x & =-2 \\
x & =+2
\end{aligned}
$$

अतः संकुल का सूत्र $\mathrm{K}_{2}\left[\mathrm{Ni}(\mathrm{CN})_{4}\right]$ है।
इसी प्रकार

(iii) $\left[\mathrm{Cr}(\mathrm{en})_{3}\right] \mathrm{Cl}_{3}$
(iv) $\left[\mathrm{Pt}\left(\mathrm{NH}_{3}\right) \mathrm{BrCl}\left(\mathrm{NO}_{2}\right)\right]^{-}$
(v) $\left[\mathrm{PtCl}_{2}(\mathrm{en})_{2}\right]\left(\mathrm{NO}_{3}\right)_{2}$
(vi) $\mathrm{Fe}_{4}\left[\mathrm{Fe}(\mathrm{CN})_{6}\right]_{3}$
:::

:::

:::question{number="5.2" kind="additional_exercise" id="it_5.2" topic="IUPAC names of coordination compounds" corrections_applied="1"}
#### अतिरिक्त प्रश्न 5.2

:::prompt
निम्नलिखित उपसहसंयोजन यौगिकों के IUPAC नाम लिखिए-
(i) $\left[\mathrm{Co}\left(\mathrm{NH}_{3}\right)_{6}\right] \mathrm{Cl}_{3}$
(ii) $\left[\mathrm{Co}\left(\mathrm{NH}_{3}\right)_{5} \mathrm{Cl}\right] \mathrm{Cl}_{2}$
(iii) $\mathrm{K}_{3}\left[\mathrm{Fe}(\mathrm{CN})_{6}\right]$
(iv) $\mathrm{K}_{3}\left[\mathrm{Fe}\left(\mathrm{C}_{2} \mathrm{O}_{4}\right)_{3}\right]$
(v) $\mathrm{K}_{2}\left[\mathrm{PdCl}_{4}\right]$
(vi) $\left[\mathrm{Pt}\left(\mathrm{NH}_{3}\right)_{2} \mathrm{Cl}\left(\mathrm{NH}_{2} \mathrm{CH}_{3}\right)\right] \mathrm{Cl}$
:::

:::figure{src="images/fig_5_29.jpg" id="fig_5_29"}
:::

:::figure{src="images/fig_5_30.jpg" id="fig_5_30"}
:::

:::figure{src="images/fig_5_31.jpg" id="fig_5_31"}
:::

:::figure{src="images/fig_5_32.jpg" id="fig_5_32"}
:::

:::figure{src="images/fig_5_33.jpg" id="fig_5_33"}
:::

:::figure{src="images/fig_5_34.jpg" id="fig_5_34"}
:::

:::solution{label="हल"}
(i)
माना कि Co की ऑक्सीकरण अवस्था $x$ है

$$
\begin{gathered}
x+(0) 6+(-1) \times 3=0 \\
x+0-3=0 \Rightarrow x=+3
\end{gathered}
$$

अतः इस संकुल का नाम हेक्साऐमीनकोबाल्ट (III) क्लोराइड है

(ii)
$$
\begin{gathered}
x+(0) \times 5+(-1) \times 1+(-1) \times 2=0 \\
x+0-3=0 \\
x=+3
\end{gathered}
$$
अत: संकुल का नाम पेन्टाऐमीनक्लोराइडोकोबाल्ट (III) क्लोराइड है।

(iii)
माना कि Fe की ऑक्सीकरण अवस्था $x$ है।

$$
\begin{gathered}
(+1) 3+x+(-1) 6=0 \\
+3+x-6=0 \\
x=+3
\end{gathered}
$$

अतः संकुल का नाम पोटैशियमहेक्सासायनोफेरेट (III) है।

(iv)
माना कि Fe की ऑक्सीकरण अवस्था $x$ है।

$$
(+1) 3+x+(-2) 3=0
$$

$\left[\because\right.$ ऑक्सेलेट आयन पर $\left(\mathrm{C}_{2} \mathrm{O}_{4}^{2-}\right)-2$ आवेश है। $]$

$$
\begin{aligned}
3+x-6 & =0 \\
x & =+3
\end{aligned}
$$

अतः संकुल का नाम पोटैशियमट्राइआक्सैलेटोफेरेट (III) है।

(v)
माना कि Pd की ऑक्सीकरण अवस्था $x$ है।

$$
\begin{aligned}
(+1) 2+x+(-1) 4 & =0 \\
2+x-4 & =0 \\
x & =+2
\end{aligned}
$$

अत: संकुल का नाम पोटैशियमटेट्राक्लोरोडोपैलेडेट (II) है।

(vi)
माना कि Pt की ऑक्सीकरण अवस्था $x$ है।

$$
\begin{aligned}
x+(0) 2+(-1) \times 1+0 & +(-1) \times 1=0 \\
x+0-1+0-1 & =0 \\
x-2 & =0 \\
x & =+2
\end{aligned}
$$

अतः संकुल का नाम डाइऐमीनक्लोरोडो (मेथिल ऐमीन) प्लैटिनम (II) क्लोराइड है।
:::

:::

:::question{number="5.3" kind="additional_exercise" id="it_5.3" topic="Isomerism in coordination complexes"}
#### अतिरिक्त प्रश्न 5.3

:::prompt
निम्नलिखित संकुलों द्वारा प्रदर्शित समावयवता का प्रकार बतलाइए तथा इन समावयवों की संरचनाएं बनाइए।
(i) $\mathrm{K}\left[\mathrm{Cr}\left(\mathrm{H}_{2} \mathrm{O}\right)_{2}\left(\mathrm{C}_{2} \mathrm{O}_{4}\right)_{2}\right]$
(ii) $\left[\mathrm{Co}(\text { en })_{3}\right] \mathrm{Cl}_{3}$
(iii) $\left[\mathrm{Co}\left(\mathrm{NH}_{3}\right)_{5}\left(\mathrm{NO}_{2}\right)\right]\left(\mathrm{NO}_{3}\right)_{2}$
(iv) $\quad\left[\mathrm{Pt}\left(\mathrm{NH}_{3}\right)\left(\mathrm{H}_{2} \mathrm{O}\right) \mathrm{Cl}_{2}\right]$
:::

:::figure{src="images/fig_5_35.jpg" id="fig_5_35"}
(समपक्ष समावयव में समान लिग्रेण्ड पास-पास जुड़े रहते हैं जबकि विपक्ष समावयव में एक-दूसरे के विपरीत स्थान पर उपस्थित होते हैं)।
:::

:::figure{src="images/fig_5_36.jpg" id="fig_5_36"}
:::

:::figure{src="images/fig_5_37.jpg" id="fig_5_37"}
:::

:::figure{src="images/fig_5_38.jpg" id="fig_5_38"}
समपक्ष
:::

:::figure{src="images/fig_5_39.jpg" id="fig_5_39"}
विपक्ष
:::

:::solution{label="हल"}
(i) $\mathrm{K}\left[\mathrm{Cr}\left(\mathrm{H}_{2} \mathrm{O}\right)_{2}\left(\mathrm{C}_{2} \mathrm{O}_{4}\right)_{2}\right]$ या $\mathrm{K}\left[\mathrm{Cr}\left(\mathrm{H}_{2} \mathrm{O}\right)_{2}(\mathrm{OX})_{2}\right]$ ( जहाँ, $\mathrm{ox}=$ ऑक्सेलेट आयन)

(a) यह ज्यामितीय समावयवता दर्शाता है अर्थात् इसके दो समावयी समपक्ष तथा विपक्ष समावयव होते हैं।


(b) समपक्ष समावयव समतिति तल की अनुपस्थिति के कारण ध्रुवण समावयवता अर्थात् $d$-तथा $1$-रूप भी दर्शाता है।


(ii) $\left[\mathrm{Co}(\mathrm{en})_{3}\right] \mathrm{Cl}_{3}$ इसके दो धुवण समावयव ( $d$-तथा $t$-रूप) हैं। अर्थात्

(iii) $\left[\mathrm{Co}\left(\mathrm{NH}_{3}\right)_{5}\left(\mathrm{NO}_{2}\right)\right]\left(\mathrm{NO}_{3}\right)_{2}$
इसके निम्नलिखित आयनिक समावयव तथा बंधनी समावयव अस्तित्व में होंगे।

आयनन समावयव
$\left[\mathrm{Co}\left(\mathrm{NH}_{3}\right)_{5}\left(\mathrm{NO}_{2}\right)\right]\left(\mathrm{NO}_{3}\right)_{2}$ तथा $\left[\mathrm{Co}\left(\mathrm{NH}_{3}\right)_{5}\left(\mathrm{NO}_{3}\right)\right]\left(\mathrm{NO}_{2}\right)\left(\mathrm{NO}_{3}\right)$
क्योंकि ये आयनित होकर विभिन्न आयन देते हैं।

$$
\begin{aligned}
{\left[\mathrm{Co}\left(\mathrm{NH}_{3}\right)_{5}\left(\mathrm{NO}_{2}\right)\right]\left(\mathrm{NO}_{3}\right)_{2} \xrightarrow{\text { आयनन }}\left[\mathrm{Co}\left(\mathrm{NH}_{3}\right)_{5}\left(\mathrm{NO}_{2}\right)\right]^{2+} } & \\
& +2 \mathrm{NO}_{3}^{-}
\end{aligned}
$$

$$
\begin{aligned}
{\left[\mathrm{Co}\left(\mathrm{NH}_{3}\right)_{5}\left(\mathrm{NO}_{3}\right)\right]\left(\mathrm{NO}_{2}\right)\left(\mathrm{NO}_{3}\right) \xrightarrow{\text { आयनन }} } & {\left[\mathrm{Co}\left(\mathrm{NH}_{3}\right)_{5} \mathrm{NO}_{3}\right]^{2+} } \\
+ & \mathrm{NO}_{2}^{-}+\mathrm{NO}_{3}^{-}
\end{aligned}
$$

बंघनी समावयव
$\left[\mathrm{Co}\left(\mathrm{NH}_{3}\right)_{5}\left(\mathrm{NO}_{2}\right)\right]\left(\mathrm{NO}_{3}\right)_{2}$ तथा $\left[\mathrm{Co}\left(\mathrm{NH}_{3}\right)_{5} \mathrm{ONO}\right]\left(\mathrm{NO}_{3}\right)_{2}$

(iv) $\left[\mathrm{Pt}\left(\mathrm{NH}_{3}\right)\left(\mathrm{H}_{2} \mathrm{O}\right) \mathrm{Cl}_{2}\right]$ इसके निम्नलिखित दो ज्यामितीय समावयवी अस्तित्व में होगें।
:::

:::

:::question{number="5.4" kind="additional_exercise" id="it_5.4" topic="Ionisation isomerism"}
#### अतिरिक्त प्रश्न 5.4

:::prompt
इसका प्रमाण दीजिए कि $\left[\mathrm{Co}\left(\mathrm{NH}_{3}\right)_{5} \mathrm{Cl}\right] \mathrm{SO}_{4}$ तथा $\left[\mathrm{Co}\left(\mathrm{NH}_{3}\right)_{5}\left(\mathrm{SO}_{4}\right)\right] \mathrm{Cl}$ आयनन समावय हैं।
:::

:::solution{label="हल"}
दोनों यौगिकों को दो अलग-अलग परखनली में जल में घोलेंगे तथा दोनों के लिए निम्न परीक्षण करेंगे

परीक्षण । परखनलियों में $\mathrm{BaCl}_{2}$ विलयन डालने पर एक यौगिक का सफेद अवक्षेप बनाता है, यह यौगिक में $\mathrm{SO}_{4}^{2-}$ आयनों की उपस्थिति को दर्शाता है। जबकि दूसरा यौगिक सफेद अवक्षेप नहीं देता है, यह यौगिक में $\mathrm{SO}_{4}^{2-}$ आयनों की अनुपस्थिति को दर्शाता है।

$$
\underset{\text { [I] }}{\left[\mathrm{Co}\left(\mathrm{NH}_{3}\right)_{5} \mathrm{Cl}\right] \mathrm{SO}_{4}}+\mathrm{BaCl}_{2}(\mathrm{aq}) \longrightarrow\left[\mathrm{Co}\left(\mathrm{NH}_{3}\right)_{5} \mathrm{Cl}\right] \mathrm{Cl}_{2}+\underset{\text { सफेद अवक्षेप }}{\mathrm{BaSO}_{4}(s)}
$$

परीक्षण II दोनों परखनलियों में $\mathrm{AgNO}_{3}$ विलयन मिलाने पर, केवल यौगिक (II) सफेद अवक्षेप देता है जबकि प्रति आयन $\mathrm{Cl}^{-}$आयनों की अनुपस्थिति के कारण (I) सफेद अवक्षेप नहीं देता है।

$$
\begin{aligned}
& {\left[\mathrm{Co}\left(\mathrm{NH}_{3}\right)_{5} \mathrm{SO}_{4}\right] \mathrm{Cl}+\mathrm{AgNO}_{3}(\mathrm{aq}) \longrightarrow\left[\mathrm{Co}\left(\mathrm{NH}_{3}\right)_{5} \mathrm{SO}_{4}\right] \mathrm{NO}_{3}+\underset{\text { सफेद अवक्षेप }}{\mathrm{AgCl}(\mathrm{~s})}} \\
& {\left[\mathrm{Co}\left(\mathrm{NH}_{3}\right)_{5} \mathrm{Cl}\right] \mathrm{SO}_{4}+\mathrm{AgNO}_{3}(\mathrm{aq}) \longrightarrow \text { कोई अभिक्रिया नहीं }}
\end{aligned}
$$

यह दोनों परीक्षण पुष्टि करते हैं कि दिये गये दोनों यौगिक एक युगल आयनन समावयव हैं।
:::

:::

:::question{number="5.5" kind="additional_exercise" id="it_5.5" topic="Valence bond theory and magnetism" corrections_applied="1"}
#### अतिरिक्त प्रश्न 5.5

:::prompt
संयोजकता आबंध सिद्धांत के आधार पर समझाइए कि वर्ग समतलीय संरचना वाला $\left[\mathrm{Ni}(\mathrm{CN})_{4}\right]^{2-}$ आयन प्रतिचुंबकीय है तथा चतुष्फलकीय ज्यामिति वाला $\left[\mathrm{NiCl}_{4}\right]^{2-}$ आयन अनुचुंबकीय है।
:::

:::figure{src="images/fig_5_40.jpg" id="fig_5_40"}
:::

:::solution{label="हल"}
$\left[\mathrm{Ni}\left(\mathrm{CN}_{4}\right]^{2-}\right.$ आयन ${ }_{28} \mathrm{Ni}$ परमाणु का बाहरी विन्यास $=3 d^{8} 4 s^{2}$
( $\mathrm{CN}^{-}$प्रबल क्षेत्र लिगेण्ड होने के कारण घातु आयन के $d$-इलेक्ट्रॉनों का युग्मन कर देता है।)

$$
\left[\mathrm{Ni}(\mathrm{CN})_{4}\right]^{2-} \text { आयन }=\begin{array}{|l|l|l|l|l|}
\hline 1 \mathrm{l} & 1 \mathrm{l} & 1 \mathrm{l} & 1 \mathrm{l} & \times \mathrm{x} \\
\hline & \underbrace{\mathrm{CN}^{-}} & \underbrace{\begin{array}{|c|}
\hline \times \mathrm{x} \\
\hline
\end{array}} \begin{array}{|c|l|}
\hline \times \mathrm{x} & \times \times \\
\hline
\end{array} \\
\hline
\end{array}
$$
$d s p^{2}$ संकरण
⇒ वर्ग समतली संरचना

यह संकुल आयन प्रतिचुम्बकीय है क्योंकि इसमें अयुग्मित इलेक्ट्रॉन नहीं है।
${ }_{28} \mathrm{Ni}$ परमाणु का बाहरी विन्यास $=3 d^{8} 4 s^{2}$

$$
\begin{aligned}
&{ }_{28} \mathrm{Ni} \text { का बाहरी विन्यास }=\begin{array}{|l|l|l|l|l|}
\hline 1 l & 1 l & 1 l & 1 & 1 \\
\hline
\end{array} \begin{array}{|l|l|}
4 s^{2} \\
\hline
\end{array}{ }^{\frac{3 d^{8}}{4 d^{8}}} \\
& \mathrm{Ni}^{2+} \text { आयन }=\begin{array}{|l|l|l|l|}
\hline 1 l & 1 l & 1 l & 1 \\
\hline
\end{array} \\
& \hline
\end{aligned}
$$

$\left(\mathrm{Cl}^{-}\right.$दुर्बल क्षेत्र लिगेण्ड होने के कारण इलेक्ट्रॉनों का युग्मन नहीं कर सकता है।)

$$
\left[\mathrm{NiCl}_{4}\right]^{2-}=\begin{array}{|l|l|l|l|l|}
\hline 1 \mathrm{l} & 1 \mathrm{l} & 1 \mathrm{l} & 1 & 1 \\
\hline
\end{array} \underbrace{\begin{array}{|c|}
\hline \times \mathrm{x} \\
\hline \mathrm{Cl}^{-} \\
\hline
\end{array} \begin{array}{|c|c|}
\hline \times \times & \mathrm{Cl}^{-} \mathrm{Cl}^{-} \mathrm{Cl}^{-} \\
\hline \times \times & \times \times \\
\hline
\end{array}}
$$
$s p^{3}$ संकरण
यह संकुल आयन अनुचुम्बकीय है क्योंकि इसमें दो अयुग्मित इलेक्ट्रॉन हैं। ⇒ चतुष्फलकीय संरचना
:::

:::

:::question{number="5.6" kind="additional_exercise" id="it_5.6" topic="Paramagnetism of tetrahedral complexes"}
#### अतिरिक्त प्रश्न 5.6

:::prompt
$\left[\mathrm{NiCl}_{4}\right]^{2-}$ अनुचुंबकीय है जबकि $\left[\mathrm{Ni}(\mathrm{CO})_{4}\right]$ प्रतिचुंबकीय है यद्यपि दोनों चतुष्फलकीय है। क्यों?
:::

:::figure{src="images/fig_5_41.jpg" id="fig_5_41"}
:::

:::figure{src="images/fig_5_42.jpg" id="fig_5_42"}
:::

:::solution{label="हल"}
Ni परमाणु का बाहरी विन्यास $=3 d^{8} 4 s^{2}$
$\left[\mathrm{NiCl}_{4}\right]^{2-}$ संकुल में $\mathrm{Ni}^{2+}$ आयन का विन्यास $3 d^{8}$ है ($\mathrm{Cl}^{-}$दुर्बल क्षेत्र लिगेण्ड होने के कारण इलेक्ट्रॉनों का युग्मन नहीं कर सकते हैं)। अतः इसमें दो अयुग्मित इलेक्ट्रॉन उपस्थित रहते हैं, जिसके कारण यह अनुचुम्बकीय है। संकरण $s p^{3}$ है, अतः संकुल चतुष्फलकीय है।
$\mathrm{Ni}\left(\mathrm{CO}_{4}\right.$ संकुल में Ni की ऑक्सीकरण अवस्था शून्य है तथा इलेक्ट्रॉनिक विन्यास $3 d^{8} 4 \mathrm{~s}^{2}$ है। CO (प्रबल क्षेत्र लिगेण्ड) लिगेण्ड की उपस्थिति में $4 s$ इलेक्ट्रॉन $3 d$ कक्षक में चले जाते हैं 3d-कक्षक में सभी इलेक्ट्रॉनों का युग्मन हो जाता है। संकरण में $4 s$ तथा $4 p$-कक्षक भाग लेते है। अतः संकुल चतुष्फलकीय लेकिन प्रतिचुम्बकीय है।
Ni परमाणु का बाहरी विन्यास $=3 d^{8} 4 s^{2}$
:::

:::

:::question{number="5.7" kind="additional_exercise" id="it_5.7" topic="Magnetism of iron complexes" corrections_applied="1"}
#### अतिरिक्त प्रश्न 5.7

:::prompt
$\left[\mathrm{Fe}\left(\mathrm{H}_{2} \mathrm{O}\right)_{6}\right]^{3+}$ प्रबल अनुचुंबकीय है जबकि $\left[\mathrm{Fe}(\mathrm{CN})_{6}\right]^{3-}$ दुर्बल अनुचुंबकीय। समझाइए।
:::

:::figure{src="images/fig_5_43.jpg" id="fig_5_43"}
:::

:::figure{src="images/fig_5_44.jpg" id="fig_5_44"}
:::

:::figure{src="images/fig_5_45.jpg" id="fig_5_45"}
:::

:::solution{label="हल"}
$26 \mathrm{Fe}$ का बाहरी विन्यास $=3 d^{6}, 4 s^{2}$
दोनों संकुलों में $\mathrm{Fe}, \mathrm{Fe}^{3+}$ आयन के रूप में है।
अतः, $\mathrm{Fe}^{3+}$ आयन का विन्यास $=3 d^{5} 4 s^{0}$
$\mathrm{CN}^{-}$लिगेण्ड (प्रबल क्षेत्र लिगेण्ड) की उपस्थिति में, $3 d$ कक्षक में इलेक्ट्रॉनों का युग्मन होने के पश्चात् एक इलेक्ट्रॉन अयुग्मित रहता है। $d^{2} s p^{3}$ संकरण होने के साथ आन्तरिक कक्षक संकुल बनता है।
$\mathrm{H}_{2} \mathrm{O}$ (दुर्बल क्षेत्र लिगेण्ड) की उपस्थिति में $3 d$-इलेक्ट्रॉन युग्मित नहीं हो पाते है। अत:
$s p^{3} d^{2}$ संकरण के साथ बाह्य कक्षक संकुल बनता है इसमें पाँच अयुग्मित इलेक्ट्रॉन होते हैं, जिससे यह प्रबल अनुचुम्बकीय है।
:::

:::

:::question{number="5.8" kind="additional_exercise" id="it_5.8" topic="Inner and outer orbital complexes" corrections_applied="2"}
#### अतिरिक्त प्रश्न 5.8

:::prompt
समझाइए कि $\left[\mathrm{Co}\left(\mathrm{NH}_{3}\right)_{6}\right]^{3+}$ एक आंतरिक कक्षक संकुल है जबकि $\left[\mathrm{Ni}\left(\mathrm{NH}_{3}\right)_{6}\right]^{2+}$ एक बाह्य कक्षक संकुल है।
:::

:::figure{src="images/fig_5_46.jpg" id="fig_5_46"}
:::

:::figure{src="images/fig_5_47.jpg" id="fig_5_47"}
:::

:::solution{label="हल"}
$\left[\mathrm{Co}\left(\mathrm{NH}_{3}\right)_{6}\right]^{3+}$ में, $\mathrm{Co}+3$ ऑक्सीकरण अवस्था में है तथा इसका विन्यास $3 d^{6}$ है।
$\mathrm{NH}_{3}$ की उपस्थिति में $3 d$-कक्षकों में इलेक्ट्रॉन युग्मित हो जाते हैं। शेष दो रिक्त $3 d$-कक्षक $d^{2} s p^{3}$ संकरण में भाग लेकर आन्तरिक कक्षक संकुल बनाते हैं।
$\left[\mathrm{Co}\left(\mathrm{NH}_{3}\right)_{6}\right]^{3+}$ संकुल आयन में
$\left[\mathrm{Ni}\left(\mathrm{NH}_{3}\right)_{6}\right]^{2+}$ में Ni की ऑक्सीकरण अवस्था $+2$ तथा विन्यास $d^{8}$ है।
चूँकि यहाँ $(n-1) d$ कक्षक उपलब्ध नहीं है अत: $n d$-कक्षक संकरण में भाग लेते हैं। इस प्रकार बना संकुल, बाह्य कक्षक संकुल कहलाता है।
:::

:::

:::question{number="5.9" kind="additional_exercise" id="it_5.9" topic="Unpaired electrons in square planar complex" corrections_applied="1"}
#### अतिरिक्त प्रश्न 5.9

:::prompt
वर्ग समतली $\left[\mathrm{Pt}(\mathrm{CN})_{4}\right]^{2-}$ आयन में अयुग्मित इलेक्ट्रॉनों की संख्या बतलाइए।
:::

:::figure{src="images/fig_5_48.jpg" id="fig_5_48"}
:::

:::solution{label="हल"}
${ }_{78} \mathrm{Pt}$ आवर्त सारणी के वर्ग-10 में उपस्थित है। इसका बाह्य विन्यास $5 d^{9} 6 s^{1}$ है।
वर्ग समतली संरचना के लिए, इस आयन में $d s p^{2}$ संकरण होगा। अतः $5 d$ कक्षकों में इलेक्ट्रॉनों के युग्मन होने से एक $d$-कक्षक रिक्त रहता है जो $d s p^{2}$ संकरण में भाग लेता है। इस प्रकार इस आयन में एक भी अयुग्मित इलेक्ट्रॉन नहीं है।
:::

:::answer
**उत्तर:** इस आयन में एक भी अयुग्मित इलेक्ट्रॉन नहीं है (अयुग्मित इलेक्ट्रॉनों की संख्या शून्य है)।
:::

:::

:::question{number="5.10" kind="additional_exercise" id="it_5.10" topic="Crystal field theory and unpaired electrons"}
#### अतिरिक्त प्रश्न 5.10

:::prompt
क्रिस्टल क्षेत्र सिद्धांत को प्रयुक्त करते हुए समझाइए कि कैसे हेक्साएक्वा मैंगनीज (II) आयन में पाँच अयुगलित इलैक्ट्रॉन हैं जबकि हेक्सासायनो आयन में केवल एक ही अयुगलित इलेक्ट्रॉन हैं।
:::

:::figure{src="images/fig_5_49.jpg" id="fig_5_49"}
:::

:::figure{src="images/fig_5_50.jpg" id="fig_5_50"}
:::

:::solution{label="हल"}
$\mathrm{Mn}(\mathrm{II})$ आयन का विन्यास $3 d^{5}$ है। $\mathrm{H}_{2} \mathrm{O}$ लिगेण्ड (दुर्बल क्षेत्र लिगेण्ड) की उपस्थिति में इन पाँच इलेक्ट्रॉनों का वितरण $t_{2 g}^{3} e_{g}^{2}$ है। अर्थात् पाँचों इलेक्ट्रॉन अयुग्मित रहकर उच्च प्रचक्रण संकुल बनाते हैं।

हाँलाकि, $\mathrm{CN}^{-}$लिगेण्ड की उपस्थिति में (जोकि प्रबल क्षेत्र लिगेण्ड है), इन इलेक्ट्रॉनों का वितरण $t_{2 \mathrm{~g}}^{5} e_{\mathrm{g}}^{0}$ है अर्थात् दो $t_{2 \mathrm{~g}}$ कक्षकों में अयुग्मित इलेक्ट्रॉन तथा एक $t_{2 \mathrm{~g}}$-क्षक में $1$ अयुग्मित इलेक्ट्रॉन है। इस प्रकार निम्न प्रचक्रण संकुल बनता है।
:::

:::
