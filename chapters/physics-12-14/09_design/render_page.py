# -*- coding: utf-8 -*-
"""Stage 9 render script for physics-12-14 (Semiconductor Electronics -
Materials, Devices and Simple Circuits / अर्धचालक इलेक्ट्रॉनिकी): reads
08_tag/structured.hi.json, hand-templates it onto the house style
(solutions-chapter-1.html's exemplar), directly modeled on
chapters/physics-12-13/09_design/render_page.py (the most recently proven
stage-9 script in this repo).

This chapter has exactly 9 items (3 examples + 6 exercises), 2 parts (both
in ex_14.4), and exactly ONE figure in the whole final item set
(images/fig_physics-12-14_16.jpg, ex_14.4's V-I characteristic graph - see
stage 2's report: of 38 mathpix images and ~60 solutions-manual table rows,
all but this one figure belonged either to general chapter exposition never
tied to a specific item, or to solutions-manual content for sections this
rationalised edition cut entirely - transistors, photodiodes, logic gates).
No :::note and no :::table anywhere in this chapter's final item set, so
those branches are inherited unused (kept only so this script stays a
faithful copy of the proven reference rather than a stripped-down one that
might silently diverge in behavior it does share).
"""
...
CH = os.path.join(ROOT, "chapters", "physics-12-14")
...
def render_block_flow(flow):
    ...
    row; a nested table segment becomes its own table block. Neither is
    exercised by this chapter's own content (no figure or table nested
    inside any solution block - this chapter's one figure is a bare
    item-level block, handled separately in render_item) but kept for
    parity with the reference script."""
    parts = []
    for seg in flow:
        if seg["type"] == "text":
...
def render_answer_flow(answer_list):
    """Text-only join (strips any leading '**उत्तर:**' markdown prefix -
    stage 8 preserves it as-is from structured.hi.md, stripped here at
    render time).

    Also used for item/part PROMPTS, not just answers - an MCQ item's own
    prompt (q_14.1/14.3/14.4/14.5) is written as a stem line, a blank line,
    then one labelled option per line ("(a) ...\\n(b) ...\\n..."). Plain
    HTML collapses all of that embedded whitespace to a single space, which
    ran every option together into one dense paragraph on the first render
    (caught on this chapter's own stage-9 screenshot read-back - nothing
    was WRONG or missing, just hard to read). Converting the source's own
    blank-line/single-newline breaks to <br><br>/<br> after math conversion
    (never before - a raw "\\n" inside a $...$ span must stay untouched
    until latex.convert has already consumed it) restores one option per
    line without touching a single character of the actual text.
    """
    if not answer_list:
        return ""
    texts = []
...
def render_answer_table_block(answer_list):
    """Not exercised by this chapter (no table answers) - kept for parity."""
...
def render_item(item, color_idx):
    ...
    if item["parts"]:
        # An item can ALSO carry its own item-level solution_blocks alongside
        # non-empty parts - ex_14.4 (this chapter's only multi-part item)
        # genuinely does: a shared-setup concept ("हम डायोड के अभिलाक्षणिक
        # वक्र को...") that applies to BOTH parts, confirmed at stage 8 as
        # real shared context, not a duplicate re-narration of either part's
        # own working. Unlike physics-12-13 (where this branch was a no-op),
        # it fires here - and it must render BEFORE the parts, not after:
        # this is the METHOD/context each part's own working depends on, so
        # showing it after the parts' calculations reads backwards. Caught
        # on the stage-9 screenshot read-back (the "जानकारी" pill was
        # rendering below both parts a/b instead of above them).
        if item["solution_blocks"]:
    ...
    # item-level combined answer alongside parts - ex_14.4's own item-level
    # "answer" is empty (its two parts each carry their own separate boxed
    # answer instead), so this never fires; kept for parity with the
    # reference script / a future rerun.
