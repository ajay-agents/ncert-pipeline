# -*- coding: utf-8 -*-
"""Stage 9 render script for physics-12-7: reads 08_tag/structured.hi.json,
hand-templates it onto the house style (solutions-chapter-1.html's exemplar,
concretely modeled on chapters/physics-12-6/09_design/final.hi.html), and
...
CH = os.path.join(ROOT, "chapters", "physics-12-7")
...
def render_flow_lines(text):
    ...
    for m in _DISPLAY_RE.finditer(text):
        ...
        if '<div class="s92' in env_html:
            lines.append(f'<div class="{"s92 s60"}">' + _strip_outer_div(env_html) + "</div>" if False else env_html)
        else:
...
def _strip_outer_div(html):
    return html


def render_block_flow(flow):
    """A block's flow list (this chapter: always exactly one {type:text}
    segment, no nested figures) -> list of <div class="s27">...</div>
    strings for its .s26 wrapper."""
    parts = []
    for seg in flow:
        if seg["type"] == "text":
...
def render_solution_blocks(blocks, color, final_answer_flow):
    if not blocks:
        return ""
    rows = []
    last_content_children = None  # (list to append answer box into)
    for b in blocks:
        btype = b["type"]
        if btype in ("concept", "formula", "step"):
            ...
            rows.append(("row_bare", "".join(render_block_flow(b.get("flow", []))), None))
    ...
    if final_answer_flow and last_stageable is None:
        # no stageable block at all (shouldn't happen in this chapter, but
        # guard anyway) - append a bare answer row
        ans_text_plain = plain_len(final_answer_flow)
        cls = answer_box_class(color, ans_text_plain)
...
def render_item(item, color_idx, is_last_decimal_2digit_variant):
    ...
    # item-level combined answer (only meaningful when the item ALSO has
    # parts, each with their own answer already shown above - this is a
    # real, distinct combined summary, per step_8/PROMPT.md, not a duplicate)
    if item["parts"] and item["answer"]:
...
def build():
    ...
    for it in examples:
        out.append(render_item(it, color_idx, None))
        color_idx = (color_idx + 1) % 4
        out.append(f'<div class="{COLORS[color_idx]["divider_before"]}"></div>\n')
    # remove the trailing divider after the last example (section header follows instead)
