...
NOARG_SYMBOL = {
    ...
    # this chapter (Continuity and Differentiability) only ever uses \circ
    # for function composition (g \circ h), never a degree sign - confirmed
    # by grepping every \circ occurrence in 06_simplify/chapter.simplified.hi.md
    # before assuming the maths-12-4/Physics-style degree-sign mapping
    # transfers; padded with non-breaking spaces like the other binary
    # operators (times/cdot/div) so "g \circ h" never wraps mid-expression.
    "circ": NBSP + "\u2218" + NBSP,
    "pm": NBSP + "\u00b1" + NBSP,
    "cup": NBSP + "\u222a" + NBSP,
    "prime": "\u2032",
...
# "lim" behaves exactly like sin/cos/... below (bare operator name, then the
# main loop's own generic "_" handling renders the following \lim _{x \to c}
# subscript) - this chapter (Continuity) uses \lim 204 times, its single
# most common command, so this is not optional coverage.
        ...
        if c == "\\":
            ...
            if m:
                ...
                if cmd in ("mathbf", "boldsymbol"):
                    # \mathbf{R}/\boldsymbol{R} - the real-number-set symbol
                    # in this chapter's domain-of-continuity items (6
                    # occurrences); bold, otherwise rendered like any other
                ...
                if cmd in OPERATOR_NAMES:
                    j2 = j
                    # optional ^{-1} or ^{...} - only look past whitespace
                    # for this; if there's no "^" there, the whitespace must
                    # NOT be consumed (it's the ordinary separating space
                    # before the operator's own argument, e.g. "\sin c" -
                    # advancing i past it here silently glued "sin"+"c"
                    # into "sinc" on every trig/log call in this chapter,
                    # caught for real via the design screenshot read-back).
                    sup = None
                    while j2 < n and s[j2] == " ":
                        j2 += 1
                    if j2 < n and s[j2] == "^":
                ...
                # name) rather than silently dropping it. A silent drop turns
                # a missing-coverage gap into invisible lost content that no
                # gate catches; printing the literal name at least surfaces
                # in the mandatory manual read-back as obviously wrong text,
                # per step_9/PROMPT.md's own documented history of this
                # exact failure mode recurring across chapters/subjects.
                out.append(cmd)
                i = j
                continue
