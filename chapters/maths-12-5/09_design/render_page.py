# -*- coding: utf-8 -*-
"""Stage 9 render script for maths-12-5 (सांतत्य तथा अवकलनीयता /
Continuity and Differentiability), adapted from maths-12-4's own working
render_page.py/latex.py (the most recent, most similar prior chapter),
plus every trap documented in step_9/PROMPT.md. Section list rewritten
for this chapter's seven exercises (प्रश्नावली 5.1-5.7) instead of
maths-12-4's five; two genuine :::note blocks (ex_5.17/ex_5.32's टिप्पणी
remarks, both handled the same way as maths-12-4's single one).
NEW IN THIS CHAPTER, first real exercise of this path in the maths
sub-series: genuine figures (6, all real continuity/discontinuity
graphs, not decorative) - 5 as item-level `item['figures']` (rendered by
render_item below, which maths-12-4 never needed since it had zero) and
1 (ex_5.9's fig_5.3) as a bare top-level :::figure inside :::solution,
exercising render_solution_stack's figure-group branch for real for the
first time. Images copied into this directory's own images/ subfolder
(fig_maths-12-5_3.jpg through _8.jpg) so `src="images/..."` resolves
relative to this page, per step_9/PROMPT.md's own warning about a bare
filename 404ing silently."""
import json
import re
import sys
import os
...
def render_prompt(text):
    ...
    - but first split out any embedded markdown pipe-table (this chapter
    has none, but the check is cheap and keeps parity with the proven
    pattern from prior chapters in case a later correction introduces
    one). Returns the full wrapped HTML (callers must NOT wrap this in
    another <p>)."""
    if not text:
        return ''
    lines = text.split('\n')
...
def render_table_html(table_md):
    ...
    This chapter has no tables at all, but kept for parity with the
    proven pattern - a flow-reader that only ever joins type:"text"
    segments silently drops these, the same documented blind spot as a
...
def render_flow_html(flow, text_renderer):
    ...
    instead of being silently dropped. (This chapter has no figures inside
    any item's flow either - both extracted images sit in theory sections
    - so no figure-in-flow handling is needed here.)"""
    out = []
    for seg in flow:
...
# Note pills: exactly 2 fixed colors in the exemplar (solutions-chapter-1),
# not cycled by item color - recall gets the warm/yellow .s74, caution
# gets the pink/red .s75, per step_9/PROMPT.md's own mapping; tip falls
# back to .s74 since the exemplar has no third distinct color for it.
...
def group_blocks(blocks):
    ...
    block with no `stage` key (a :::note or a bare top-level :::figure)
    gets its own group kind instead of being silently skipped - this
    chapter has a genuine :::note (ex_4.10's टिप्पणी) that maths-12-3
    never exercised this path against, per step_9/PROMPT.md's own
    documented trap about note/figure blocks needing their own group kind
    checked BEFORE the stage dispatch, not folded into it or dropped."""
    groups = []
    for b in blocks:
        if b['type'] == 'note':
...
def render_solution_stack(blocks, answer_text, color_idx):
    ...
    # per step_9/PROMPT.md, never silently drop the answer box this way.
    ...
    for i, g in enumerate(groups):
        ...
        if g.get('kind') == 'figure':
            # this chapter has none (both extracted images sit in theory
            # sections, never inside an item), but handled defensively -
...
def strip_redundant_enumeration(item_prompt, parts):
    ...
    prompt). A part label already wrapped in parens from stage 7
    (label="(i)") must not be double-wrapped when displayed - handled at
    the call site (render_item), not here."""
...
def render_item(item, color_idx, badges):
    ...
    # not nested in a step's flow) - first real exercise of this path in
    # the maths sub-series (maths-12-4 had none). Rendered right after the
    # prompt, same figure-row markup render_solution_stack's own
    # bare-figure-block branch uses.
    ...
    if parts_list:
        for p in parts_list:
            ...
            # chapter ("(i)", "(ii)") - don't wrap it again (see step_9/
            # PROMPT.md's documented double-paren trap).
...
def build_page():
    ...
    sections = [
        ...
        ('प्रश्नावली 5.1', [it for it in ITEMS if it['id'].startswith('q_5.1.')]),
        ('प्रश्नावली 5.2', [it for it in ITEMS if it['id'].startswith('q_5.2.')]),
        ('प्रश्नावली 5.3', [it for it in ITEMS if it['id'].startswith('q_5.3.')]),
        ('प्रश्नावली 5.4', [it for it in ITEMS if it['id'].startswith('q_5.4.')]),
        ('प्रश्नावली 5.5', [it for it in ITEMS if it['id'].startswith('q_5.5.')]),
        ('प्रश्नावली 5.6', [it for it in ITEMS if it['id'].startswith('q_5.6.')]),
        ('प्रश्नावली 5.7', [it for it in ITEMS if it['id'].startswith('q_5.7.')]),
...
<title>सांतत्य तथा अवकलनीयता · अध्याय 5 &mdash; प्रश्न एवं हल</title>
