...
The pattern of iron filings permits us to plot the magnetic field lines*. This is shown both for the bar-magnet and the current-
...
carrying solenoid in Fig. 5.2. For comparison refer to the Chapter 1, Figure 1.14(d). Electric field lines of an electric dipole are also displayed in Fig. 5.2(c). The magnetic field lines are a visual and intuitive realisation of the magnetic field. Their properties are:
...
\subsection*{5.2.2 Bar magnet as an equivalent solenoid}
...
Comparison of Eqs. (5.1), (5.2) and (5.3) with the corresponding equations for electric dipole (Chapter 1), suggests that magnetic field at large distances due to a bar magnet of magnetic moment m can be obtained from the equation for electric field due to an electric dipole of dipole moment $\mathbf{p}$, by making the following replacements:

$$
\mathbf{E} \rightarrow \mathbf{B}, \mathbf{p} \rightarrow \mathbf{m}, \frac{1}{4 \pi \varepsilon_{0}} \rightarrow \frac{\mu_{0}}{4 \pi}
$$

In particular, we can write down the equatorial field $\left(\mathbf{B}_{\mathbf{E}}\right)$ of a bar magnet at a distance $r$, for $r \gg l$, where $l$ is the size of the magnet:

$$
\mathbf{B}_{E}=-\frac{\mu_{0} \mathbf{m}}{4 \pi r^{3}}
$$

Likewise, the axial field $\left(\mathbf{B}_{\mathrm{A}}\right)$ of a bar magnet for $r \gg l$ is:
...
| External Field: Energy | -p•E | $\boldsymbol{-} \mathbf{m} \boldsymbol{\cdot} \mathbf{B}$ |
...
For instance for the configuration $\mathrm{Q}_{3}$ for which Q is along the perpendicular bisector of the dipole P, the magnetic moment of Q is Thus,

(b) (i) $\mathrm{PG}_{3}, \mathrm{PO}_{6}$ (stable); (ii) $\mathrm{PQ}_{5}, \mathrm{PQ}_{4}$ (unstable)

(a) $\mathrm{PQ}_{1}$ and $\mathrm{PO}_{2}$
(c) $\mathrm{PO}_{6}$
...
Consider a small vector area element $\Delta \mathbf{S}$ of a closed surface S as in Fig. 5.5. The magnetic flux through ÄS is defined as $\Delta \phi_{B}=\mathbf{B} \cdot \Delta \mathbf{S}$, where B is the field at $\Delta \mathbf{S}$. We divide $S$ into many small area elements and calculate the individual flux through each. Then, the net flux $\phi_{B}$ is,

$$
\phi_{B}=\sum_{a l l} \Delta \phi_{B}=\sum_{a l l} \mathbf{B} \cdot \Delta \mathbf{S}=0
$$
...
Solution
(a) No. The magnetic force is always normal to B (remember magnetic force $=q \mathbf{v} \quad \mathbf{B}$ ). It is misleading to call magnetic field lines as lines of force.
(b) Gauss's law of magnetism states that the flux of B through any closed surface is always zero $\int_{\mathrm{S}} \mathbf{B} . \Delta \mathbf{s}=0$.
...
If the interior of the solenoid is filled with a material with non-zero magnetisation, the field inside the solenoid will be greater than $\mathbf{B}_{0}$. The net B field in the interior of the solenoid may be expressed as
...
where $\mu_{0}$ is the same constant (permiability of vacuum) that appears in Biot-Savart's law.
...
where $\mathbf{H}$ has the same dimensions as $\mathbf{M}$ and is measured in units of $\mathrm{A} \mathrm{m}^{-1}$. Thus, the total magnetic field B is written as
...
| :--- | :--- | :--- |
| $\begin{aligned} & -1 \leq \chi<0 \\ & 0 \leq \mu_{r}<1 \end{aligned}$ | $\begin{gathered} 0<\chi<\varepsilon \\ 1<\mu_{r}<1+\varepsilon \\ \mu>\mu_{0} \end{gathered}$ | $\begin{aligned} & \chi \gg 1 \\ & \mu_{r} \gg 1 \\ & \mu \gg \mu_{0} \end{aligned}$ |
...
Figure 5.7(a) shows a bar of diamagnetic material placed in an external magnetic field. The field lines are repelled or expelled and the field inside the material is reduced. In most cases, this reduction is slight, being one

![](images/fig_physics-12-5_7.jpg)
FIGURE 5.7 Behaviour of magnetic field lines near a
(a) diamagnetic,
...
microscope after sprinkling a liquid suspension of powdered ferromagnetic substance of samples. This motion of suspension can be observed. Fig. 5.8(b) shows the situation when the domains have aligned and amalgamated to form a single 'giant' domain.
...
2. When a bar magnet of dipole moment $\mathbf{m}$ is placed in a uniform magnetic field B,
...
3. Consider a bar magnet of size 1 and magnetic moment $\mathbf{m}$, at a distance $r$ from its mid-point, where $r \gg 1$, the magnetic field $\mathbf{B}$ due to this bar is,
$$
\begin{aligned}
\mathbf{B} & =\frac{\mu_{0} \mathbf{m}}{2 \pi r^{3}} & & \text { (along axis) } \\
& =-\frac{\mu_{0} \mathbf{m}}{4 \pi r^{3}} & & \text { (along equator) }
\end{aligned}
$$
4. Gauss's law for magnetism states that the net magnetic flux through any closed surface is zero
$$
\phi_{B}=\sum_{\substack{\text { all area } \\ \text { elements } \Delta \mathbf{s}}} \mathbf{B} \cdot \Delta \mathbf{S}=0
$$
...
$$
The magnetisation $\mathbf{M}$ of the material is its dipole moment per unit volume. The magnetic field B in the material is,
$$
\mathbf{B}=\mu_{0}(\mathbf{H}+\mathbf{M})
$$
6. For a linear material $\mathbf{M}=\chi \mathbf{H}$. So that $\mathbf{B}=\mu \mathbf{H}$ and $\chi$ is called the magnetic susceptibility of the material. The three quantities, $\chi$, the relative magnetic permeability $\mu_{\mathrm{r}}$, and the magnetic permeability $\mu$ are related as follows:
$$
\begin{aligned}
& \mu=\mu_{0} \mu_{r} \\
& \mu_{r}=1+\chi
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| Permeability of free space | $\mu_{0}$ | Scalar | [MLT ${ }^{-2} \mathrm{~A}^{-2}$ ] | $\mathrm{T} \mathrm{m} \mathrm{A}^{-1}$ | $\mu_{0} / 4 \pi=10^{-7}$ |
| Magnetic field, Magnetic induction, Magnetic flux density | B | Vector | $\left[\mathrm{MT}^{-2} \mathrm{~A}^{-1}\right]$ | T (tesla) | $10^{4} \mathrm{G}$ (gauss) $=1 \mathrm{~T}$ |
| Magnetic moment | m | Vector | $\left[\mathrm{L}^{-2} \mathrm{~A}\right]$ | $\mathrm{A} \mathrm{m}^{2}$ |  |
| Magnetic flux | $\phi_{\mathrm{B}}$ | Scalar | $\left[\mathrm{ML}^{2} \mathrm{~T}^{-2} \mathrm{~A}^{-1}\right]$ | W (weber) | $W=\mathrm{T} \mathrm{m}^{2}$ |
| Magnetisation | M | Vector | $\left[\mathrm{L}^{-1} \mathrm{~A}\right]$ | $\mathrm{A} \mathrm{m}^{-1}$ | Magnetic moment |
| Magnetic intensity Magnetic field strength | H | Vector | $\left[\mathrm{L}^{-1} \mathrm{~A}\right]$ | $\mathrm{A} \mathrm{m}^{-1}$ | $\mathbf{B} \boldsymbol{=} \mu_{0} \boldsymbol{(} \mathbf{H} \boldsymbol{+} \mathbf{M} \boldsymbol{)}$ |
| Magnetic susceptibility | $\chi$ | Scalar | - | - | $\mathbf{M} \boldsymbol{=} \boldsymbol{\chi} \mathbf{H}$ |
| Relative magnetic permeability | $\mu_{r}$ | Scalar | - | - | $\mathbf{B}=\mu_{0} \mu_{r} \mathbf{H}$ |
| Magnetic permeability | $\mu$ | Scalar | [MLT ${ }^{-2} \mathrm{~A}^{-2}$ ] | $\mathrm{T} \mathrm{m} \mathrm{A}^{-1} \mathrm{N} \mathrm{A}^{-2}$ | $\begin{aligned} & \mu=\mu_{0} \mu_{r} \\ & \mathbf{B}=\mu \mathbf{H} \end{aligned}$ |
...
5.1 A short bar magnet placed with its axis at 30° with a uniform external magnetic field of 0.25 T experiences a torque of magnitude equal to $4.5 \times 10^{-2} \mathrm{~J}$. What is the magnitude of magnetic moment of the magnet?
5.2 A short bar magnet of magnetic moment $\mathrm{m}=0.32 \mathrm{JT}^{-1}$ is placed in a uniform magnetic field of 0.15 T. If the bar is free to rotate in the plane of the field, which orientation would correspond to its (a) stable, and (b) unstable equilibrium? What is the potential energy of the magnet in each case?
...
5.5 A bar magnet of magnetic moment $1.5 \mathrm{~J} \mathrm{~T}^{-1}$ lies aligned with the direction of a uniform magnetic field of 0.22 T.
    (a) What is the amount of work required by an external torque to turn the magnet so as to align its magnetic moment: (i) normal to the field direction, (ii) opposite to the field direction?
    (b) What is the torque on the magnet in cases (i) and (ii)?
5.6 A closely wound solenoid of 2000 turns and area of cross-section $1.6 \times 10^{-4} \mathrm{~m}^{2}$, carrying a current of 4.0 A, is suspended through its centre allowing it to turn in a horizontal plane.
...
5.7 A short bar magnet has a magnetic moment of 0.48 $\mathrm{J} \mathrm{T}^{-1}$. Give the direction and magnitude of the magnetic field produced by the magnet at a distance of 10 cm from the centre of the magnet on (a) the axis, (b) the equatorial lines (normal bisector) of the magnet.

[^0]:    Example 5.5 A solenoid has a core of a material with relative permeability 400. The windings of the solenoid are insulated from the core and carry a current of 2A. If the number of turns is 1000 per metre, calculate (a) $H$, (b) $M$, (c) $B$ and (d) the magnetising current $I_{m}$.
