...
GREEK = {
    ...
    "rho": "\u03c1", "sigma": "\u03c3", "tau": "\u03c4",
    # added for this chapter (Moving Charges and Magnetism) - angular
    # speed/frequency notation used constantly (ex_4.10(d), sol_4.19,
    # sol_4.20) that neither prior physics chapter's own content needed.
    "omega": "\u03c9", "nu": "\u03bd",
    # added for this chapter (Moving Charges and Magnetism) - a moment-of-
    # inertia symbol mathpix OCRs inconsistently across ex_4.10(d)'s own
    # derivation (as a bare "9", as \mathscr{g}, and as \vartheta - three
    # different mis-renderings of what's clearly meant to be one consistent
    # symbol). Rendering each exactly as its own source token, not unifying
    # them (that would be altering content, not this stage's job) - just
    # making sure \vartheta itself doesn't fall through as literal text.
    "vartheta": "\u03d1",
}
...
NOARG_SYMBOL = {
    ...
    # added for this chapter (ex_4.10(d)'s angular-speed derivation
    # integrates both sides: \int_{0}^{\omega f} ... \mathrm{d}\omega). The
    # generic _{...}/^{...} handler elsewhere in this file picks up the
    # following limits as ordinary sub/superscripts once this symbol itself
    # is emitted, matching how \sin/\cos already work.
    "int": "\u222b",
    ...
    "ll": NBSP + "\u226a" + NBSP,        # much-less-than, e.g. d \ll R (sol_4.16)
    "simeq": NBSP + "\u2243" + NBSP,     # asymptotically-equal-to, e.g. \simeq 0.02 (sol_4.27)
    "cong": NBSP + "\u2245" + NBSP,      # added for this chapter (Magnetism and
                                          # Matter) - "approximately equal to",
                                          # e.g. ex_5.5's M \cong 8x10^5 A/m;
                                          # missing from every prior chapter's
                                          # own converter since none of them
                                          # used \cong.
        ...
        if c == "\\":
            ...
            if m:
                ...
                if cmd == "vec":
                    ...
                    continue
                if cmd == "mathscr":
                    # \mathscr{g} - one of the three inconsistent mathpix
                    # renderings of ex_4.10(d)'s moment-of-inertia symbol
                    # (see the GREEK/vartheta comment above). No true script
                    # font in this page's stack; italicize like any other
                    # single-variable symbol rather than drop it.
                    arg, j = _read_arg(s, j)
                    out.append(f"<i>{_convert_inner(arg)}</i>")
                    i = j
                    continue
