# -*- coding: utf-8 -*-
"""Stage 9 render script for physics-12-3-en (Current Electricity, English
medium), adapted from physics-12-2-en's own working render_page.py/latex.py
(the most recent prior chapter in this same subject+language lineage), plus
every trap documented in step_9/PROMPT.md.

Differences from physics-12-2-en, the chapter this was adapted from:
- **First chapter this lineage has hit a real :::table** (sol_3.17's
  manganin-resistor current/voltage observations table, in the item's own
  PROMPT, not a solution block). `render_table_html` and its dispatch
  branches already existed in physics-12-2-en's copy ("kept for parity",
  never exercised there) but had NO matching CSS at all (.s99/.s100 were
  referenced by the render function but undefined) - added real table CSS
  (.s98 scrollable wrapper, .s99/.s100, teal header row matching the pill
  accent palette) and wrapped the table in .s98 here.
- **First chapter to need `\\Omega`** (the ohm resistance symbol) - entirely
  missing from physics-12-1-en's/physics-12-2-en's GREEK table, since a
  potential-and-capacitance chapter never happened to use it; a Current
  Electricity chapter uses it on nearly every line. Added to NOARG_SYMBOL
  as an upright (non-italicized) capital omega, same treatment as \\Delta/
  \\Phi, not run through the Greek-italics pass since it's a unit symbol,
  not a variable name.
- q_3.7's solution has a genuine bare :::figure directly inside :::solution
  (not nested in a step) - group_blocks/render_solution_stack's existing
  figure-before-stage-dispatch check (inherited, never exercised on a real
  case in physics-12-2-en) is what correctly renders this, not a new fix.
- 18 distinct figures across this chapter (vs. 9 in physics-12-2-en),
  several items with 2-7 figures each (q_3.7: 3, sol_3.20: 7).
"""
import json
import re
import sys
    ...
    for raw_line in protected.split('\n'):
        ...
        while i < n:
            ...
            if c == '.':
                ...
                # a '.' immediately followed by a closing bracket - e.g. a
                # trailing parenthetical remark, "...comes out the same.)" -
                # is not itself the sentence boundary; splitting there
                # strands the lone ')' as its own one-character "sentence"
                # on the next line. Let the closing bracket ride along with
                # this sentence instead of starting a new one.
                if nxt in ')]}':
                    buf.append(nxt)
                    i += 1
...
def render_table_html(table_md):
    ...
    physics-12-1-en/physics-12-2-en never had a table at all; this
    chapter's sol_3.17 is the first real exercise - a flow-reader that
    only ever joins type:"text" segments would silently drop it (the same
    root cause step_9/PROMPT.md documents for a dropped mid-solution
    figure). Wrapped in a scrollable .s98 div so a wide table never
    overflows the page on a narrow viewport or in the printed PDF."""
    ...
    return f'<div class="s98"><table class="s99">{"".join(rows_html)}</table></div>'
...
def collect_nested_figure_srcs(blocks):
    ...
    flows (directly, or nested inside a step/formula/concept/note's own
    flow) - used to avoid re-rendering the SAME figure a second time in
    the item-level 'figures before the solution' row. The JSON's
    item-level `figures` array is a flat, comprehensive inventory of every
    figure anywhere in the item (per step_8/PROMPT.md's own schema) - it
    is NOT a guarantee that those figures are siblings sitting before the
    solution; for an item like q_2.1, both of its figures are nested
    inside separate steps' own flows instead, and rendering them again
    from `item['figures']` before the solution stack even starts would
    duplicate each one on the page (caught for real via stage 9's visual
    QA screenshot)."""
    srcs = set()
    for b in blocks or []:
...
def render_item(item, color_idx, badges):
    ...
    if not skip_item_prompt:
        if parts_list:
            ...
            # text. This chapter's two multi-part items (ex_3.1, ex_3.2)
            # have no table/figure in their own item-level prompt flow -
            # sol_3.17's table lives on a non-multi-part item and takes the
            # `else` branch below instead, which handles it directly.
            if prompt_text_use:
    ...
    # skip any figure that's ALSO nested inside a solution-block flow
    # (item-level or any part's), or it renders twice on the page.
    ...
    if parts_list:
        # Some items (physics-12-5-en's q_5.2/5.5/5.6/5.7) have parts whose
        # OWN solution_blocks/answer are empty, with the real combined
        # answer addressing every part together living only at item level -
        # a legitimate shape (confirmed non-empty at stage 8), not the
        # unrelated duplicate-trailing-solution artifact. Render each
        # part's own prompt as normal, then - only when NO part actually
        # carried its own solution - fall back to the item-level
        ...
        # dropped just because it lives one level up from where a part
        # loop naturally looks.
...
def build_page():
    ...
    sections = [
        ...
        # physics-12-3-en keeps 15 genuine orphan solutions (older-edition
        # exercises the 2023 rationalisation dropped from the current
        # textbook) as their own additional_exercise items, under their
        # own section - physics-12-2-en never carried any orphan into its
        # final chapter, so this kind never needed a section here before.
