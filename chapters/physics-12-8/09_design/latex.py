# -*- coding: utf-8 -*-
"""LaTeX -> hand-rendered HTML converter for physics-12-8's stage 9 page.
Modeled closely on chapters/physics-12-7/09_design/latex.py (itself modeled
on the conventions in step_9/PROMPT.md and chapters/physics-12-6/09_design/
final.hi.html) - fractions as .s31/.s32/.s33, sqrt as &radic;+overline span,
multi-row aligned blocks as .s92/.s93, "=" glued to a following frac/sqrt via
.s90, non-breaking spaces around ~/×/·/=, degree signs as <sup>°</sup>.
...
\\AA \\Omega \\because \\begin{aligned} \\cdot \\frac \\gamma \\hat \\lambda
\\left \\right \\mathbf \\mathrm \\mu \\omega \\phi \\pi \\quad \\right
\\sigma \\sin \\sqrt \\text \\therefore \\times \\varepsilon
- physics-12-7's converter already covers most of this (varepsilon/lambda
via its GREEK dict, sin via its trig tuple); this chapter is new only in
\\AA (angstrom, q_8.3's "6800 \\AA"), \\gamma and \\sigma (not in the
inherited GREEK dict - \\gamma appears as a plain symbol, e.g. "$\\gamma$
किरण", not just inside a unit), and \\hat (unit-vector caret, e.g.
\\hat{\\mathbf{k}}, \\hat{i} - q_8.1's B direction reasoning and its own
answer). No set-theory/sum/oint/cases/gathered - only one environment
(aligned) is used anywhere in this chapter.
"""
import re
