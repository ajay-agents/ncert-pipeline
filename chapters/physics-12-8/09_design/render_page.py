# -*- coding: utf-8 -*-
"""Stage 9 render script for physics-12-8: reads 08_tag/structured.hi.json,
hand-templates it onto the house style (solutions-chapter-1.html's exemplar,
concretely modeled on chapters/physics-12-7/09_design/render_page.py, which
is itself modeled on chapters/physics-12-6/09_design/final.hi.html), and
writes 09_design/final.hi.html + final.hi.css + images/.
"""
...
CH = os.path.join(ROOT, "chapters", "physics-12-8")
...
def render_table_html(md_text):
    ...
    the base stylesheet's own .s61/.s62/.s63/.s64 (already present for
    exactly this purpose, unused by any prior chapter's own content)."""
...
def render_answer_flow(answer_list):
    """Text-only join (strips the leading '**उत्तर:**' markdown prefix - the
    box styling itself already conveys 'this is the answer'). Never called
    on an answer list that also contains a table segment - see
    render_answer_table_block for that shape instead."""
    if not answer_list:
        return ""
    texts = []
...
def render_answer_table_block(answer_list):
    """An answer whose result IS a table (q_8.9's per-band photon-energy
    summary) doesn't fit the small pill-shaped boxed-answer echo (.s37 and
    friends are sized for a short single-line value) - render it as its own
    labelled block below the solution instead of trying to squeeze a 7-row
    table into that pill."""
...
def render_item(item, color_idx):
    ...
    badge_html = (f'<div class="{color["badge"]}"><div class="{color["qtag"]}">Q</div>'
                  ...
                  f'<div class="{color["topic"]}">{esc(item["topic"])}</div></div>')
    ...
    # item-level combined answer alongside parts (not used by this chapter -
    # no item here has both non-empty parts AND its own item-level answer -
    # kept for parity with the reference script / a future rerun).
