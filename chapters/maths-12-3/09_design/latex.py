...
adapted for maths-12-3 (Matrices) from maths-12-2's own working converter.
Modeled on the established stage-9 pattern (see step_9/PROMPT.md for every
...
New in this chapter: genuine bracketed-matrix rendering (\left[\begin{array}
{colspec}...\end{array}\right]) as a real CSS grid with drawn brackets
(.s94-.s98) - this chapter's dominant content shape (854 occurrences in the
source), completely different from a bare \begin{array} used for equation
alignment (which the pre-existing generic \begin handler already covers
adequately and still handles unchanged for the ~9 non-bracketed cases).
"""
import re
