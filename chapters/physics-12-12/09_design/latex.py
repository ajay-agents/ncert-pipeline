# -*- coding: utf-8 -*-
"""LaTeX -> hand-rendered HTML converter for physics-12-11's stage 9 page.
Copied from chapters/physics-12-10/09_design/latex.py (itself modeled on
physics-12-9/12-8/12-7/12-6's own converters) - fractions as .s31/.s32/.s33,
sqrt as &radic;+overline span, multi-row aligned/array/gathered blocks as
.s92/.s93 (handled generically via \\begin{env}...\\end{env} regardless of
env name, so 'gathered' -- used once in this chapter, q_11.9 -- needs no
special-casing), "=" glued to a following frac/sqrt via .s90, non-breaking
spaces around ~/×/·/=, degree signs as <sup>&deg;</sup>.

This chapter's own LaTeX vocabulary (grepped from 07_format/structured.hi.md,
per the documented "grep before trusting a reused converter" discipline --
Dual Nature of Radiation and Matter is photon/de-Broglie arithmetic, heavy on
Greek-letter physical quantities): \\Rightarrow \\because \\therefore \\frac
\\left \\right \\mathrm \\text \\tan \\times \\quad \\prime \\max
\\lambda \\theta \\phi \\rho \\varphi, plus \\begin{aligned}/\\begin{array}/
\\begin{gathered}. Everything except \\rho and \\varphi was already in the
inherited tables (Rightarrow/because/therefore/frac/left/right/mathrm/text/
tan/times/quad/prime/lambda/theta/phi all already handled; \\max already
deliberately falls through to its bare-name text, which is the CORRECT
rendering for a subscript label like "v_{\\max}", not a gap). \\rho (momentum
p, used constantly: "p=h/\\lambda", "\\rho=hv/c") was NOT in the GREEK table
-- every prior chapter's own content happened never to need it -- and would
have fallen through to the generic "unknown command: print its bare name"
path, rendering the literal word "rho" instead of ρ on nearly every momentum
line in this chapter (ex_11.3, q_11.4, q_11.11). \\varphi (an OCR/typesetting
variant of \\phi appearing once, q_11.9's "$40.75\\times10^{-20}-\\varphi_{0}$"
mid-derivation, referring to the exact same work-function variable defined
two lines earlier via "\\phi_{0}") was equally unhandled -- mapped to the
same glyph as phi so the two spellings of the same physical quantity render
identically on the page, rather than one silently reading "varphi0" in plain
text next to the other's correctly-rendered φ0.
"""
import re
...
GREEK = {
    ...
    # Added for physics-12-11 -- see module docstring: \rho (momentum, used
    # constantly throughout this chapter's de Broglie/photon-momentum
    # derivations) and \varphi (an OCR variant spelling of \phi appearing
    # once, q_11.9, for the same work-function variable defined via \phi_0
    # two lines earlier) were both missing from every prior chapter's table.
    "rho": "ρ", "varphi": "φ",
    # Added for physics-12-12 (Atoms): \alpha -- the alpha PARTICLE of the
    # Geiger-Marsden/Rutherford scattering experiment (ex_12.2's "$\alpha$-
    # कण", q_12.6's Sommerfeld fine-structure-constant "\alpha") -- was
    # missing from every prior chapter's table (none of them happened to
    # need it), and fell through to the generic "unknown command: print its
    # bare name" path, rendering the literal word "alpha" instead of α on
    # every one of these lines. Caught for real at this chapter's own
    # manual read-back screenshot review.
    "alpha": "α",
}
...
def _protect_envs(s):
    ...
    same-name backreference regex).

    A same-name backreference regex (\\begin{(?P<env>...)}...\\end{(?P=env)},
    non-greedy) mismatches the instant an environment is nested INSIDE
    another environment of the SAME name (q_12.6's own part (a): an outer
    \\begin{aligned}...\\end{aligned} whose one row itself contains a nested
    \\begin{aligned}...\\end{aligned}, a genuine Mathpix OCR artifact of the
    source PDF, not something authored by hand). The non-greedy body match
    stops at the FIRST '\\end{aligned}' it finds scanning forward from the
    OUTER '\\begin{aligned}' -- which is the INNER environment's own closing
    tag, not the outer's -- leaving the true outer '\\end{aligned}' dangling
    with no '\\begin' left to pair it with. That orphaned '\\end{aligned}'
    then falls through the generic "unknown command" path in the main
    converter loop (\\end -> bare word "end", the brace-stripped "aligned"
    printed as literal running text) and shows up as the literal word
    "endaligned" glued onto the page, with a matching stray "beginaligned"
    left over from the inner environment's own un-paired opening tag.

    Fix: scan every \\begin{name}/\\end{name} token in the string in order,
    track nesting DEPTH (not name-equality) with a simple counter, and only
    ever protect a span whose \\begin started at depth 0 and whose \\end
    brought the depth back to 0 -- i.e. the outermost, correctly-paired
    block, regardless of what environment name is nested how many levels
    inside it. Nested environments inside that body are left untouched here
    and handled later, when _render_env recurses into that body with its own
    fresh _protect_envs call (see _render_env's own docstring)."""
    stash = []
    out = []
    pos = 0
    depth = 0
    top_start = None       # index into `s` where the current top-level \begin starts
    top_name = None
    top_body_start = None  # index just after the top-level \begin{name}'s closing brace
    ...
    for m in _ENV_TOKEN_SCAN_RE.finditer(s):
        ...
        if kind == "begin":
            ...
            if depth == 0:
                continue  # unmatched stray \end -- leave as-is, not our problem here
            depth -= 1
            if depth == 0:
        ...
        if c == "\\":
            ...
            if not m:
                ...
                if s[i:i + 2] == "\\ ":
                    ...
                    continue
                if s[i:i + 2] == "\\_":
                    # Escaped underscore -- a literal "_" character, used in
                    # this chapter's own content as a fill-in-the-blank
                    # marker inside a math span (q_12.1's five MCQ parts each
                    # have a run of four "\_" in the question text, e.g.
                    # "...साइज़ से $\_\_\_\_$ होता है।"). Not a subscript
                    # operator (that's a BARE "_", handled separately below) --
                    # but the command regex above only matches "\[a-zA-Z]+",
                    # so "\_" fell through unhandled and the backslash was
                    # silently dropped; the bare "_" left behind was then
                    # picked up by the unrelated subscript-operator branch
                    # (c == "_"), which read the FOLLOWING "\_" as its own
                    # "content" and rendered an empty <sub> instead of a
                    # visible blank -- the whole fill-in-the-blank marker
                    # vanished from the page with nothing to show a student
                    # where to write an answer. Render each escaped
                    # underscore as a literal underscore character.
                    out.append("_")
                    i += 2
                    continue
                if s[i:i + 2] == "\\,":
...
def _render_env(env, body):
    ...
    # Protect any NESTED \begin{...}...\end{...} block (e.g. a \begin{array}
    # annotation sitting inside one row of an outer \begin{aligned}) BEFORE
    # splitting on the outer environment's own "\\" row separator. Without
    # this, the naive re.split(r"\\\\", body) below cuts straight through
    # the nested environment's own internal "\\" row breaks (it has no idea
    # they belong to a different, still-unprotected \begin/\end pair),
    # stranding a fragment like "\begin{array}{rl}...=0.040 \mathrm{~kg}" as
    # the tail of one outer row and the array's \end{array}/\right) as the
    # head of a later one. Each such fragment then gets passed to convert()
    # on its own, where \begin/array/rl/end (no longer looking like a real
    # \begin{env}...\end{env} pair once truncated) all fall through the
    # generic "unknown command: print its bare name" path -- caught for
    # real on physics-12-11's q_11.10a ("\begin{aligned}...\quad\left(
    # \begin{array}{rl}\because\quad m&=0.040\mathrm{~kg}\\v&=1\mathrm{~km}/
    # \mathrm{s}\\&=1000\mathrm{~m}/\mathrm{s}\end{array}\right)\\&=1.66e-35
    # \mathrm{~m}\end{aligned}"), which rendered as literal garbled text
    # ("beginarrayrl", "endarray") instead of the intended parenthetical
    # aside. Fix: protect nested envs first (turning them into opaque
    # tokens the "\\" split can't be fooled by), split the now-safe body,
    # then convert each row via _convert_inner directly against the SAME
    # nested-envs stash (bypassing convert()'s own top-level
    # _rewrite_divisions/_protect_envs call, which would otherwise look the
    # token up in a fresh, empty envs list of its own and raise/blank out --
    # the same "call the inner converter directly against the right stash"
    # pattern already used for \mathrm/\text's own content).
