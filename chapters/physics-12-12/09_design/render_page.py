# -*- coding: utf-8 -*-
"""Stage 9 render script for physics-12-12 (Atoms / परमाणु): reads
...
chapters/physics-12-11/09_design/render_page.py (the most recently proven
and complete of this pipeline's stage-9 scripts, itself modeled on
physics-12-10's), and writes 09_design/final.hi.html + final.hi.css.

This chapter has no :::note, :::table or :::figure content anywhere in its
final item set (13 items: 4 examples + 9 exercises, either a purely
qualitative Rutherford/Thomson-model argument or a numeric Bohr-model
given/formula/step derivation; confirmed at stage 2 that none of the
chapter's 8 mathpix images are referenced inside any example/exercise),
so the note/table/figure branches below are inherited unused, kept only so
this script stays a faithful copy of the proven reference rather than a
stripped-down one that might silently diverge in behavior it does share.
...
converter" discipline) is \\AA \\alpha \\approx \\begin \\cdot \\end \\frac
\\lambda \\left \\mathrm \\nu \\pi \\quad \\right \\sqrt \\text \\therefore
\\times \\varepsilon plus \\begin{aligned}/\\begin{array}/\\begin{gathered}
-- every one of these is already handled, unmodified, by latex.py as
inherited from physics-12-11; no new symbol support was needed.
"""
...
CH = os.path.join(ROOT, "chapters", "physics-12-12")
...
def render_item(item, color_idx):
    ...
    if item["parts"]:
        ...
        # non-empty parts (the known duplicate-solution artifact from
        # step_7/PROMPT.md / step_8/PROMPT.md) whenever stage 8 classified
        # that trailing content as genuine rather than a re-narration
        # duplicate to discard -- physics-12-10's own reference script never
        # exercised this branch (none of its items kept one), so it simply
        # never rendered item["solution_blocks"] at all whenever parts was
        # non-empty, silently dropping such content. Caught for real on this
        # chapter: ex_11.3 (a genuine synthesizing conclusion comparing both
        # parts' results) and q_11.4 (genuine given data shared by parts a/b)
        # both have a non-empty item-level solution_blocks here. Render it
        # after all parts, matching the source's own physical ordering (the
        # trailing :::solution always followed the :::part blocks in
        # structured.hi.md) -- no item-level final answer to attach here
        # (every item with parts in this chapter has its own real answer
        # only at the part level, never a separate combined one).
