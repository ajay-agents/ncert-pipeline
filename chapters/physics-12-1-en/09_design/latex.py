# -*- coding: utf-8 -*-
"""LaTeX -> HTML converter for physics-12-1-en's stage 9 page, adapted from
maths-12-5's own working converter (the most recent, most bug-fixed prior
chapter - see step_9/PROMPT.md for every documented trap this implementation
defends against): division-rewrite for bare `/`, unit/exponent bypass of
that rewrite, non-breaking spaces around ~/x/./=, `=`-glued fraction/sqrt
chains, \\left(\\right) as its own unit, \\begin{aligned}/\\begin{array}/
\\begin{gathered} row splitting via the generic \\begin handler.
